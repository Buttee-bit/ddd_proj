...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new539(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new539(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi542 = RefOutArgWrapper(None)
                inoutres543 = Utils.tryGetValue(all0_, a.name, wrapsi542)
                si = wrapsi542.value
                if (not inoutres543): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli540 = RefOutArgWrapper(None)
                    inoutres541 = Utils.tryGetValue(si, s, wrapli540)
                    li = wrapli540.value
                    if (not inoutres541): 
                        li = list()
                        si[s] = li
