...
class MoneyReferent(Referent):
    ...
    def value(self) -> float:
        ...
        if (val is None): 
            return 0
        v = 0
        wrapv2368 = RefOutArgWrapper(0)
        inoutres2369 = Utils.tryParseFloat(val, wrapv2368)
        v = wrapv2368.value
        if (not inoutres2369): 
            return 0
        return v
    ...
    def alt_value(self) -> float:
        ...
        if (val is None): 
            return None
        v = 0
        wrapv2370 = RefOutArgWrapper(0)
        inoutres2371 = Utils.tryParseFloat(val, wrapv2370)
        v = wrapv2370.value
        if (not inoutres2371): 
            return None
        return v
    ...
    def rest(self) -> int:
        ...
        if (val is None): 
            return 0
        v = 0
        wrapv2372 = RefOutArgWrapper(0)
        inoutres2373 = Utils.tryParseInt(val, wrapv2372)
        v = wrapv2372.value
        if (not inoutres2373): 
            return 0
        return v
    ...
    def alt_rest(self) -> int:
        ...
        if (val is None): 
            return None
        v = 0
        wrapv2374 = RefOutArgWrapper(0)
        inoutres2375 = Utils.tryParseInt(val, wrapv2374)
        v = wrapv2374.value
        if (not inoutres2375): 
            return None
        return v
    ...
    @staticmethod
    def _new1077(_arg1 : str, _arg2 : float) -> 'MoneyReferent':
        res = MoneyReferent()
        res.currency = _arg1
