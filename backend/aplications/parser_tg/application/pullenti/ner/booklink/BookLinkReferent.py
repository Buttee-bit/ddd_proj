...
class BookLinkReferent(Referent):
    ...
    def year(self) -> int:
        """ Год """
        year_ = 0
        wrapyear517 = RefOutArgWrapper(0)
        inoutres518 = Utils.tryParseInt(Utils.ifNotNull(self.get_string_value(BookLinkReferent.ATTR_YEAR), ""), wrapyear517)
        year_ = wrapyear517.value
        if (inoutres518): 
            return year_
        else: 
            return 0
