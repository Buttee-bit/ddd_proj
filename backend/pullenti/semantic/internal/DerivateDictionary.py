...
from backend.pullenti.semantic.internal.ExplanTreeNode import ExplanTreeNode
...
class DerivateDictionary:
    ...
    def init(self, lang_ : 'MorphLang', lazy : bool) -> bool:
        ...
        names = Utils.getResourcesNames('backend.pullenti.semantic.utils.properties', '.dat')
        for n in names: 
            if (Utils.endsWithString(n, rsname, True)): 
                inf = Utils.getResourceInfo('backend.pullenti.semantic.utils.properties', n)
                if (inf is None): 
                    continue
                with Utils.getResourceStream('backend.pullenti.semantic.utils.properties', n) as stream: 
                    stream.position = 0
                    self._m_all_groups.clear()
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos3664 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos3664)
                pos = wrappos3664.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos3668 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos3668)
            pos = wrappos3668.value
            while cou > 0: 
                wrappos3666 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos3666)
                pos = wrappos3666.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos3665 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos3665)
                    pos = wrappos3665.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos3667 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos3667)
            pos = wrappos3667.value
        self.__m_buf = wr
