    ...
    def try_parse(t : 'Token', prev : 'ComplexNumToken'=None, force : bool=False, ignore_last_suffix : bool=False) -> 'ComplexNumToken':
        ...
        while t is not None: 
            if (t.is_whitespace_before): 
                break
            nt = SingleNumToken.try_parse(t, False, False)
            if (nt is None): 
                if (((force and t.is_hiphen and not t.is_newline_before) and not t.is_newline_after and (isinstance(t.next0_, NumberToken))) and res.nums[0].vals[0].typ != SingleNumValueType.DIGIT and res.nums[0].suffix is None): 
                    nt = SingleNumToken.try_parse(t.next0_, False, False)
                    if (nt is not None): 
                        res.nums[0].end_token = t
                        res.nums[0].suffix = "-"
                if (nt is None): 
                    break
