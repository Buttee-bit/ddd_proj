...
class StatisticCollection:
    ...
    def __add_token(self, tt : 'TextToken') -> 'StatisticWordInfo':
        ...
        res = None
        for v in vars0_: 
            wrapres701 = RefOutArgWrapper(None)
            inoutres702 = Utils.tryGetValue(self.__m_items, v, wrapres701)
            res = wrapres701.value
            if (inoutres702): 
                break
        if (res is None): 
            res = StatisticWordInfo._new703(tt.lemma)
        for v in vars0_: 
    ...
    def __find_item(self, tt : 'TextToken', do_absolute : bool=True) -> 'StatisticWordInfo':
        ...
        inoutres711 = Utils.tryGetValue(self.__m_items, tt.term, wrapres710)
        ...
        if (do_absolute): 
            s = MiscHelper.get_absolute_normal_value(tt.term, False)
            if (s is not None): 
                wrapres704 = RefOutArgWrapper(None)
                inoutres705 = Utils.tryGetValue(self.__m_items, s, wrapres704)
                res = wrapres704.value
                if (inoutres705): 
                    return res
        for wff in tt.morph.items: 
            wf = Utils.asObjectOrNull(wff, MorphWordForm)
            if (wf is None): 
                continue
            wrapres708 = RefOutArgWrapper(None)
            inoutres709 = Utils.tryGetValue(self.__m_items, Utils.ifNotNull(wf.normal_case, ""), wrapres708)
            res = wrapres708.value
            if (inoutres709): 
                return res
            wrapres706 = RefOutArgWrapper(None)
            inoutres707 = Utils.tryGetValue(self.__m_items, wf.normal_full, wrapres706)
            res = wrapres706.value
            if (wf.normal_full is not None and inoutres707): 
                return res
        return None
    
    def __add_bigramm(self, b1 : 'StatisticWordInfo', b2 : 'StatisticWordInfo') -> None:
        di = None
        wrapdi714 = RefOutArgWrapper(None)
        inoutres715 = Utils.tryGetValue(self.__m_bigramms, b1.normal, wrapdi714)
        di = wrapdi714.value
        if (not inoutres715): 
            di = dict()
        ...
        if (b2.normal in di): 
            ...
            di[b2.normal] = 1
        wrapdi712 = RefOutArgWrapper(None)
        inoutres713 = Utils.tryGetValue(self.__m_bigramms_rev, b2.normal, wrapdi712)
        di = wrapdi712.value
        if (not inoutres713): 
            di = dict()
    ...
    def __get_bigrams_info(self, si1 : 'StatisticWordInfo', si2 : 'StatisticWordInfo') -> 'StatisticBigrammInfo':
        res = StatisticBigrammInfo._new716(si1.total_count, si2.total_count)
        di12 = None
        wrapdi12718 = RefOutArgWrapper(None)
        Utils.tryGetValue(self.__m_bigramms, si1.normal, wrapdi12718)
        di12 = wrapdi12718.value
        di21 = None
        wrapdi21717 = RefOutArgWrapper(None)
        Utils.tryGetValue(self.__m_bigramms_rev, si2.normal, wrapdi21717)
        di21 = wrapdi21717.value
        if (di12 is not None): 
    ...
    def get_initial_info(self, ini : str, sur : 'Token') -> 'StatisticBigrammInfo':
        ...
        if (si2 is None): 
            return None
        si1 = None
        wrapsi1719 = RefOutArgWrapper(None)
        inoutres720 = Utils.tryGetValue(self.__m_items, ini[0:0+1], wrapsi1719)
        si1 = wrapsi1719.value
        if (not inoutres720): 
            return None
        if (si1 is None): 
            return None
