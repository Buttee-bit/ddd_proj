...
class ExtOntology:
    ...
    def add(self, ext_id : object, type_name : str, definition_ : str) -> 'ExtOntologyItem':
        ...
        if (rs is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3557(ext_id, rs[0], type_name)
        if (len(rs) > 1): 
            del rs[0]
            res._refs = rs
    ...
    def add_referent(self, ext_id : object, referent : 'Referent') -> 'ExtOntologyItem':
        ...
        if (referent is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3557(ext_id, referent, referent.type_name)
        self.items.append(res)
        return res
    
    def __create_referent(self, type_name : str, definition_ : str) -> typing.List['Referent']:
        analyzer = None
        wrapanalyzer3559 = RefOutArgWrapper(None)
        inoutres3560 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapanalyzer3559)
        analyzer = wrapanalyzer3559.value
        if (not inoutres3560): 
            return None
    ...
    def refresh(self, item : 'ExtOntologyItem', definition_ : object) -> bool:
        ...
        analyzer = None
        wrapanalyzer3561 = RefOutArgWrapper(None)
        inoutres3562 = Utils.tryGetValue(self.__m_anal_by_type, item.type_name, wrapanalyzer3561)
        analyzer = wrapanalyzer3561.value
        if (not inoutres3562): 
            return False
    ...
    def _get_analyzer_data(self, type_name : str) -> 'AnalyzerData':
        a = None
        wrapa3563 = RefOutArgWrapper(None)
        inoutres3564 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapa3563)
        a = wrapa3563.value
        if (not inoutres3564): 
            return None
    ...
    def __init_hash(self) -> None:
        with self.__m_lock: 
            ...
            for it in self.items: 
                if (it.referent is not None): 
                    ont = None
                    wrapont3566 = RefOutArgWrapper(None)
                    inoutres3567 = Utils.tryGetValue(hash0_, it.referent.type_name, wrapont3566)
                    ont = wrapont3566.value
                    if (not inoutres3567): 
                        ont = IntOntologyCollection._new3565(True)
    ...
    def attach_referent(self, r : 'Referent') -> typing.List['ExtOntologyItem']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3568 = RefOutArgWrapper(None)
        inoutres3569 = Utils.tryGetValue(self.__m_hash, r.type_name, wraponto3568)
        onto = wraponto3568.value
        if (not inoutres3569): 
            return None
        li = [ ]
    ...
    def attach_token(self, type_name : str, t : 'Token') -> typing.List['IntOntologyToken']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3570 = RefOutArgWrapper(None)
        inoutres3571 = Utils.tryGetValue(self.__m_hash, type_name, wraponto3570)
        onto = wraponto3570.value
        if (not inoutres3571): 
            return None
