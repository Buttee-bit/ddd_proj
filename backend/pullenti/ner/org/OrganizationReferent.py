...
class OrganizationReferent(Referent):
    ...
    def __correct_name(self, name : str, num : int) -> str:
        ...
        if (str.isdigit(name[0]) and name.find(' ') > 0): 
            i = 0
            wrapi3102 = RefOutArgWrapper(0)
            inoutres3103 = Utils.tryParseInt(name[0:0+name.find(' ')], wrapi3102)
            i = wrapi3102.value
            if (inoutres3103): 
                if (i > 1): 
                    num.value = i
            ...
            if (i >= 0 and name[i] == '.'): 
                ...
                inoutres3104 = Utils.tryParseInt(name[i + 1:], num)
                if (i > 0 and inoutres3104 and num.value > 0): 
                    if (i < 1): 
                        return None
    ...
    def add_name(self, name : str, remove_long_gov_names : bool=True, t : 'Token'=None) -> None:
        num = 0
        wrapnum3105 = RefOutArgWrapper(0)
        s = self.__correct_name(name, wrapnum3105)
        num = wrapnum3105.value
        if (s == "ЗАКАЗЧИК"): 
            pass
        if (s is None): 
    ...
    def add_name_str(self, name : str, typ : 'OrgItemTypeToken', cou : int=1) -> None:
        ...
        if (name is None): 
            ...
            if (typ.name is not None and Utils.compareStrings(typ.name, typ.typ, True) != 0 and ((len(typ.name) > len(typ.typ) or self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                num = 0
                wrapnum3106 = RefOutArgWrapper(0)
                s = self.__correct_name(typ.name, wrapnum3106)
                num = wrapnum3106.value
            ...
            if (typ is None or typ.is_not_typ): 
                ...
                if (typ.name is not None): 
                    num = 0
                    wrapnum3107 = RefOutArgWrapper(0)
                    ss = self.__correct_name(typ.name, wrapnum3107)
                    num = wrapnum3107.value
                    if (ss is not None): 
            ...
            if (((eq_number or eq_eponyms)) and ((self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None or org0_.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                ...
                for v in self._name_vars.items(): 
                    if (typ == ReferentsEqualType.DIFFERENTTEXTS and v[1]): 
                        continue
                    b = False
                    wrapb3109 = RefOutArgWrapper(False)
                    inoutres3110 = Utils.tryGetValue(org0_._name_vars, v[0], wrapb3109)
                    b = wrapb3109.value
                    if (not inoutres3110): 
                        continue
