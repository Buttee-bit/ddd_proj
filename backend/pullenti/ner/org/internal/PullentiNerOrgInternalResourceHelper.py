...
class backend.pullentiNerOrgInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('backend.pullenti.ner.org.internal.properties', '.png;.dat')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('backend.pullenti.ner.org.internal.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('backend.pullenti.ner.org.internal.properties', n) as stream: 
    ...
    def get_string(name : str) -> str:
        arr = backend.pullentiNerOrgInternalResourceHelper.get_bytes(name)
        if (arr is None): 
            return None
