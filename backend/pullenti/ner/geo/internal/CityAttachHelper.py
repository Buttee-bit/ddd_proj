...
from backend.pullenti.ner.core.ProperNameHelper import ProperNameHelper
...
class CityAttachHelper:
    ...
    def try_define(li : typing.List['CityItemToken'], ad : 'GeoAnalyzerData', always : bool=False) -> 'ReferentToken':
        ...
        if (res is None and len(li) > 1): 
            ...
            if (res is not None and res.end_char <= li[1].end_char): 
                res = (None)
        if (res is None): 
            wrapoi1415 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try1(li, wrapoi1415, ad)
            oi = wrapoi1415.value
        if (res is None): 
            wrapoi1416 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1416, False)
            oi = wrapoi1416.value
        if (res is None): 
            wrapoi1417 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_name_exist(li, wrapoi1417, False)
            oi = wrapoi1417.value
        if (res is None): 
            res = CityAttachHelper.__try4(li)
        if (res is None and always): 
            wrapoi1418 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1418, True)
            oi = wrapoi1418.value
        if (res is None and always): 
            if (OrgItemToken.try_parse(li[0].begin_token, ad) is not None): 
                pass
            else: 
                wrapoi1419 = RefOutArgWrapper(None)
                res = CityAttachHelper.__try_name_exist(li, wrapoi1419, True)
                oi = wrapoi1419.value
    ...
    def __try1(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', ad : 'AnalyzerDataWithOntology') -> 'ReferentToken':
        ...
        if (oi.value is None): 
            ...
            rt = ReferentToken._new939(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
        elif (typ is None): 
            typ = oi.value.typ
        if (rt is None): 
    ...
    def __try_noun_name(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
            ...
            if (not li[0].doubtful): 
                ...
                if (npt is not None): 
                    if (npt.end_token.end_char > li[i1].end_char and len(npt.adjectives) > 0 and not npt.adjectives[0].end_token.next0_.is_comma): 
                        ...
                        if (len(li2) > 1): 
                            oi2 = None
                            wrapoi21423 = RefOutArgWrapper(None)
                            inoutres1424 = CityAttachHelper.__try_noun_name(li2, wrapoi21423, False)
                            oi2 = wrapoi21423.value
                            if (inoutres1424 is not None): 
                                ok = True
        ...
        res = ReferentToken._new939(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
        num = None
    ...
    def __try_name_exist(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
        ...
        city._add_misc(li[0].misc)
        city._add_misc(li[0].misc2)
        return ReferentToken._new939(city, li[0].begin_token, Utils.ifNotNull(end_tok, li[0].end_token), li[0].morph)
