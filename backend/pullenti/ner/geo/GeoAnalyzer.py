...
from backend.pullenti.ner.geo.internal.MetaGeo import MetaGeo
from backend.pullenti.ner.geo.internal.GeoAnalyzerData import GeoAnalyzerData
from backend.pullenti.ner.core.internal.PullentiNerCoreInternalResourceHelper import (
    NerCoreInternalResourceHelper,
)
...
class GeoAnalyzer(Analyzer):
    ...
    def _get_data(t: "Token") -> "GeoAnalyzerData":
        ...
        return Utils.asObjectOrNull(
            t.kit.get_analyzer_data_by_analyzer_name(GeoAnalyzer.ANALYZER_NAME),
            GeoAnalyzerData,
        )
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MetaGeo.COUNTRY_CITY_IMAGE_ID] = (
            NerCoreInternalResourceHelper.get_bytes("countrycity.png")
        )
        res[MetaGeo.COUNTRY_IMAGE_ID] = (
            NerCoreInternalResourceHelper.get_bytes("country.png")
        )
        res[MetaGeo.CITY_IMAGE_ID] = (
            NerCoreInternalResourceHelper.get_bytes("city.png")
        )
        res[MetaGeo.DISTRICT_IMAGE_ID] = (
            NerCoreInternalResourceHelper.get_bytes("district.png")
        )
        res[MetaGeo.REGION_IMAGE_ID] = (
            NerCoreInternalResourceHelper.get_bytes("region.png")
        )
        res[MetaGeo.UNION_IMAGE_ID] = (
            NerCoreInternalResourceHelper.get_bytes("union.png")
        )
        return res
    ...
    def process(self, kit: "AnalysisKit") -> None:
        from backend.pullenti.ner.geo.internal.MiscLocationHelper import (
            MiscLocationHelper,
        )
        from backend.pullenti.ner.address.internal.StreetItemToken import (
            StreetItemToken,
        )
        ...
        from backend.pullenti.ner.address.internal.AddressItemToken import (
            AddressItemToken,
        )
        from backend.pullenti.ner.geo.internal.CityAttachHelper import CityAttachHelper
        ...
        from backend.pullenti.ner.geo.internal.TerrDefineHelper import TerrDefineHelper
        ...
        if MiscLocationHelper.is_user_param_address(kit.first_token):
            ...
            while t is not None:
                if isinstance(t.get_referent(), DateReferent):
                    ...
                    if (
                        rt.begin_token.chars.is_cyrillic_letter
                        and rt.begin_token.next0_ is not None
                        and rt.begin_token.next0_.is_comma
                    ) and (isinstance(rt.begin_token.next0_.next0_, NumberToken)):
                        t = kit.debed_token(t)
                t = t.next0_
        sw = Stopwatch()
        sw.reset()
        ...
        for step in range(2):
            ...
            while True:
                ...
                if step > 0 and (isinstance(t, ReferentToken)):
                    ...
                    if (
                        (
                            ...
                            and geo_.can_be_equals(
                                t.next0_.next0_.get_referent(),
                                ReferentsEqualType.WITHINONETEXT,
                            )
                        )
                        and t.next0_.next0_.next0_ is not None
                        and t.next0_.next0_.next0_.is_char(")")
                    ):
                        rt0 = ReferentToken._new939(
                            geo_, t, t.next0_.next0_.next0_, t.morph
                        )
                        kit.embed_token(rt0)
                        t = rt0
                        continue
                    if (
                        ...
                        rt0 = ReferentToken._new939(geo_, t, t.next0_.next0_, t.morph)
                        kit.embed_token(rt0)
                        t = rt0
                        continue
                ok = False
                ...
                if (rt is None and len(cli) == 1 and cli[0].is_adjective) and cli[
                    0
                ].onto_item is not None:
                    ...
                    if tt is not None:
                        ...
                        if cli1 is not None and cli1[0].onto_item is not None:
                            g0 = Utils.asObjectOrNull(
                                cli[0].onto_item.referent, GeoReferent
                            )
                            g1 = Utils.asObjectOrNull(
                                cli1[0].onto_item.referent, GeoReferent
                            )
                            if (
                                g0 is not None and g1 is not None and g0.is_region
                            ) and g1.is_region:
                                if (
                                    g0.is_city == g1.is_city
                                    or g0.is_region == g1.is_region
                                    or g0.is_state == g1.is_state
                                ):
                                    rt = TerrDefineHelper.try_define(
                                        cli, ad, True, None, None
                                    )
                        if rt is None and cli[0].onto_item.referent.is_state:
                            if (
                                rt is None
                                and tt is not None
                                and (isinstance(tt.get_referent(), GeoReferent))
                            ) and tt.whitespaces_before_count == 1:
                                geo2 = Utils.asObjectOrNull(
                                    tt.get_referent(), GeoReferent
                                )
                                if GeoOwnerHelper.can_be_higher(
                            ...
                            if rt is None and step == 0 and tt is not None:
                                npt = MiscLocationHelper._try_parse_npt(
                                    cli[0].begin_token
                                )
                                if npt is not None and npt.end_char >= tt.begin_char:
                                    ...
                                    rt1 = (
                                        None
                                        if cits is None
                                        else CityAttachHelper.try_define(
                                            cits, ad, False
                                        )
                                    )
                                    if rt1 is not None:
                                        rt1.referent = ad.register_referent(
                                            rt1.referent
                                        )
                                        kit.embed_token(rt1)
                                        cl = cli[0].onto_item.referent.clone()
                ...
                if rt is None and len(cli) == 2:
                    ...
                    if li is not None and len(li) > 0:
                        if (
                            li[0].typ == AddressItemType.STREET
                            or li[0].typ == AddressItemType.KILOMETER
                            or li[0].typ == AddressItemType.HOUSE
                        ):
                            ad0 = StreetItemToken.try_parse(
                                cli[0].begin_token.previous, None, False, None
                            )
                            if ad0 is not None and ad0.typ == StreetItemType.NOUN:
                                ...
                                aaa = AddressItemToken.try_parse(
                                    cli[0].begin_token, False, None, None
                                )
                                if (
                                    aaa is not None
                                    and aaa.typ == AddressItemType.STREET
                                ):
                                    pass
                                else:
                                    rt = TerrDefineHelper.try_define(
                ...
                if (
                    (rt is None and len(cli) > 2 and cli[0].termin_item is None)
                    and cli[1].termin_item is None
                    and cli[2].termin_item is not None
                ):
                    cit = CityItemToken.try_parse_back(
                        cli[0].begin_token.previous, False
                    )
                    if cit is not None and cit.typ == CityItemToken.ItemType.NOUN:
                        if (
                            ...
                            and cli[3].termin_item is None
                            and cli[4].termin_item is not None
                        ) and cli[2].termin_item.canonic_text.endswith(
                            cli[4].termin_item.canonic_text
                        ):
                            pass
                        else:
                            del cli[0]
                ...
                if rt is not None:
                    if (
                        MiscLocationHelper.check_territory(rt.begin_token.previous)
                        is not None
                    ):
                    ...
                    if (
                        not MiscLocationHelper.is_user_param_address(rt)
                        and not geo_.is_city
                        and not geo_.is_state
                    ) and geo_.find_slot(
                        GeoReferent.ATTR_TYPE, "республика", True
                    ) is None:
                    ...
                    if step == 0:
                        ...
                        if t.next0_ is not None and (
                            (t.next0_.is_comma or t.next0_.is_char("("))
                        ):
                            rt1 = TerrDefineHelper.try_attach_stateusaterritory(
                                t.next0_.next0_
                            )
                            if rt1 is not None:
        ...
        while True:
            ...
            if g is None:
                continue
            if (
                t.next0_ is not None
                and t.next0_.is_char_of("(/\\")
                ...
                if g2 is not None and g2 == g:
                    ...
                    if (
                        rt2.end_token.next0_ is not None
                        and rt2.end_token.next0_.is_char_of(")/\\")
                    ):
            ...
            if t.previous.is_value("СОЮЗ", None):
                ...
                t.previous.is_value("ГОСУДАРСТВО", None)
                and t.previous.previous is not None
                and t.previous.previous.is_value("СОЮЗНЫЙ", None)
            ):
            ...
            if npt is not None and npt.end_token == t.previous:
                t0 = t0.previous
            uni = GeoReferent()
            typ = MiscHelper.get_text_value(
                t0, t.previous, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE
            )
            if typ is None:
                continue
        ...
        while True:
            ...
            if li is not None:
                ...
                if (rt) is not None:
                    ...
                    if tt is not None and (isinstance(tt.get_referent(), GeoReferent)):
                        if tt.get_referent().can_be_equals(
                            rt.referent, ReferentsEqualType.WITHINONETEXT
                        ):
                            rt.begin_token = tt
                    ...
                    if len(li) == 2 and li[0].orto_city is not None:
                        li[0].orto_city.referent = ad.register_referent(
                            li[0].orto_city.referent
                        )
                        rt1 = ReferentToken(
                            li[0].orto_city.referent,
                            li[0].begin_token,
                            li[1].begin_token.previous,
                        )
                        kit.embed_token(rt1)
                        rt.begin_token = li[1].begin_token
                        rt.end_token = li[1].end_token
                    rt.referent = Utils.asObjectOrNull(
                        ad.register_referent(rt.referent), GeoReferent
                    )
                    kit.embed_token(rt)
                    t = rt
                    is_city_before = True
                ...
                if (
                    len(li) == 1
                    and li[0].typ == CityItemToken.ItemType.NOUN
                    and li[0].value == "СЕЛЬСКОЕ ПОСЕЛЕНИЕ"
                ):
                    tt1 = li[0].end_token.next0_
                    if (
                        tt1 is not None
                        and (isinstance(tt1.get_referent(), GeoReferent))
                        and tt1.get_referent().is_region
                    ):
            ...
            if (
                tts is not None
                and len(tts) > 1
                and ((tts[0].termin_item is not None or tts[1].termin_item is not None))
            ):
        ...
        if new_cities and len(ad.local_ontology.items) > 0:
            ...
            while True:
                ...
                if geo_ is not None:
                    ...
                    rt = ReferentToken._new939(
                        geo_, li[0].begin_token, li[0].end_token, t.morph
                    )
                    if rt.begin_token == rt.end_token:
                        geo_._add_name(t.term)
                    if (
                        rt.begin_token.previous is not None
                        and rt.begin_token.previous.is_value("СЕЛО", None)
                        and geo_.is_city
                    ):
                        rt.begin_token = rt.begin_token.previous
        ...
        while True:
            ...
            if tt.whitespaces_before_count > 15:
                ...
                tt is not None and tt.next0_ is not None and tt.next0_.is_hiphen
            ) and MiscLocationHelper.is_user_param_address(tt):
            ...
            while True:
                if first_pass3983:
                    first_pass3983 = False
                else:
                    ttt = ttt.next0_
                if not (ttt is not None):
                    break
                if ttt.is_comma_and:
                    comma2 = True
                    continue
                if ttt.is_hiphen:
                    if (
                        (ttt.is_whitespace_before and ttt.is_whitespace_after)
                    ) or comma2:
                        continue
            ...
            if ttt is not None and ttt.whitespaces_before_count > 15:
                ...
                ttt is not None and ttt.next0_ is not None and ttt.next0_.is_hiphen
            ) and MiscLocationHelper.is_user_param_address(ttt):
            ...
            if geo2 is not None:
                ...
                if comma1 != comma2:
                    if geo_.is_region and geo1.is_region and geo2.is_city:
                        ok2 = True
                if (
                    geo_.is_state and geo1.is_region and geo2.is_region
                ) and GeoOwnerHelper._can_be_higher_token(tt, ttt):
                    ok2 = True
                if (
                    ok2
                    and GeoOwnerHelper._can_be_higher_token(t, tt)
                    and GeoOwnerHelper._can_be_higher_token(tt, ttt)
                ):
                    geo2.higher = geo1
                    geo1.higher = geo_
                    rt0 = ReferentToken._new939(geo1, t, tt, tt.morph)
                    kit.embed_token(rt0)
                    rt = ReferentToken._new939(geo2, rt0, ttt, ttt.morph)
                    kit.embed_token(rt)
                    t = rt
                    go_back = True
                    continue
                    ...
                    if (
                        GeoOwnerHelper._can_be_higher_token(t, tt)
                        and geo_.is_region
                        and geo1.is_city
                    ) and geo2.is_region:
                        if geo2._contains_type("район") and geo_._contains_type(
                            "район"
                        ):
                            geo2.higher = geo1
                            geo1.higher = geo_
                    ...
                    if GeoOwnerHelper._can_be_higher_token(t, ttt):
                        if (
                            geo2.find_slot(GeoReferent.ATTR_TYPE, "город", True)
                            is not None
                            and geo1.find_slot(GeoReferent.ATTR_TYPE, "район", True)
                            is not None
                            and geo_.is_region
                        ):
                            geo2.higher = geo1
                            ...
                            geo2.higher = geo_
                            geo1.higher = geo2
                            rt = ReferentToken._new939(geo1, t, ttt, tt.morph)
                            kit.embed_token(rt)
                            t = rt
                            go_back = True
                            continue
                    if GeoOwnerHelper._can_be_higher_token(
                        ttt, t
                    ) and GeoOwnerHelper._can_be_higher_token(t, tt):
                    ...
                    if GeoOwnerHelper._can_be_higher_token(tt, t):
                        geo_.higher = geo1
                        geo1.higher = geo2
                        rt0 = ReferentToken._new939(geo1, tt, ttt, tt.morph)
                        kit.embed_token(rt0)
                        rt = ReferentToken._new939(geo_, t, rt0, t.morph)
                        kit.embed_token(rt)
                        t = rt
                        go_back = True
                        continue
                    if GeoOwnerHelper._can_be_higher_token(
                        t, tt
                    ) and GeoOwnerHelper._can_be_higher_token(ttt, tt):
                        if (
                            geo1.find_slot(
                                GeoReferent.ATTR_TYPE, "муниципальный округ", True
                            )
                            is not None
                            and geo_.find_slot(GeoReferent.ATTR_TYPE, "город", True)
                            is not None
                            and geo2.find_slot(GeoReferent.ATTR_TYPE, "город", True)
                            is not None
                        ):
                            if (
                                geo2.find_slot(GeoReferent.ATTR_NAME, "МОСКВА", True)
                                is not None
                            ):
                                geo_.higher = geo1
                                geo1.higher = geo2
                ...
                if comma2:
                    continue
            if GeoOwnerHelper._can_be_higher_token(t, tt) and (
                (not GeoOwnerHelper._can_be_higher_token(tt, t) or adj)
            ):
                geo1.higher = geo_
                rt = ReferentToken._new939(geo1, t, tt, tt.morph)
                if (
                    geo1.is_city and not geo_.is_city and t.previous is not None
                ) and t.previous.is_value("СТОЛИЦА", "СТОЛИЦЯ"):
                ...
                go_back = True
                continue
            if GeoOwnerHelper._can_be_higher_token(tt, t) and (
                (not GeoOwnerHelper._can_be_higher_token(t, tt) or inp)
            ):
                if geo_.higher is None:
                    geo_.higher = geo1
                elif (
                    geo1.higher is None
                    and GeoOwnerHelper.can_be_higher(geo_.higher, geo1, None, None)
                    and not GeoOwnerHelper.can_be_higher(geo1, geo_.higher, None, None)
                ):
                ...
                if bra and tt.next0_ is not None and tt.next0_.is_char(")"):
                    tt = tt.next0_
                if (
                    (bra and geo1.is_region and geo1.higher is None)
                    and geo_.is_city
                    and MiscLocationHelper.is_user_param_address(tt)
                ):
                    tt2 = tt.next0_
                    ...
                    if isinstance(tt2, TextToken):
                        ...
                        if (
                            ter is not None
                            and ter.onto_item is None
                            and ter.termin_item is None
                        ):
                            ...
                            while tt2 is not None:
                                if tt2.is_comma:
                                    ...
                                    break
                            if tt2 is not None and (
                                isinstance(tt2.get_referent(), GeoReferent)
                            ):
                                if GeoOwnerHelper.can_be_higher(
                                    Utils.asObjectOrNull(
                                        tt2.get_referent(), GeoReferent
                                    ),
                                    geo1,
                                    None,
                                    None,
                                ):
                                    geo1.higher = Utils.asObjectOrNull(
                                        tt2.get_referent(), GeoReferent
                                    )
                                    tt = tt2
                rt = ReferentToken._new939(geo_, t, tt, t.morph)
                kit.embed_token(rt)
                t = rt
                go_back = True
                continue
        ...
        for v in ad.local_ontology.items:
            ...
            if g is None or not g.is_city:
                continue
            if (
                g.find_slot(GeoReferent.ATTR_TYPE, "городское поселение", True)
                is not None
            ):
            ...
            for s in ng.slots:
                if isinstance(s.value, GeoReferent):
                    if isinstance(s.value.tag, GeoReferent):
                        ng.upload_slot(
                            s, Utils.asObjectOrNull(s.value.tag, GeoReferent)
                        )
    ...
    def __geo_comp(x: "GeoReferent", y: "GeoReferent") -> int:
        ...
        return Utils.compareStrings(
            x.to_string_ex(True, MorphLang.UNKNOWN, 0),
            y.to_string_ex(True, MorphLang.UNKNOWN, 0),
            False,
        )
    ...
    def __try_parse_city_list_back(self, t: "Token") -> typing.List["CityItemToken"]:
        ...
        while t is not None and (
            (
                t.morph.class0_.is_preposition
                or t.is_char_of(",.")
        ...
        while tt is not None:
            if not (isinstance(tt, TextToken)):
                break
            if (
                tt.previous is not None
                and tt.previous.is_hiphen
                and (isinstance(tt.previous.previous, TextToken))
            ):
    ...
    def __try_attach_territory_before_city(
        self, t: "Token", ad: "AnalyzerDataWithOntology"
    ) -> "ReferentToken":
        from backend.pullenti.ner.geo.internal.TerrItemToken import TerrItemToken
        from backend.pullenti.ner.geo.internal.TerrDefineHelper import TerrDefineHelper
        ...
        while True:
            ...
            if (
                tits1 is not None
                and tits1[len(tits1) - 1].end_token == t
                and len(tits1) == len(tits0)
            ):
                tits0 = tits1
    ...
    def __try_attach_territory_after_city(
        self, t: "Token", ad: "AnalyzerDataWithOntology"
    ) -> "ReferentToken":
        from backend.pullenti.ner.address.internal.AddressItemToken import (
            AddressItemToken,
        ...
        if (
            t.next0_ is None
            or not t.next0_.is_comma
            or t.next0_.whitespaces_after_count > 1
        ):
            return None
        tt = t.next0_.next0_
        while tt is not None:
        ...
        if ait is None:
            return None
        if (
            ait.typ == AddressItemType.STREET
            and not ait.is_doubt
            and ait.ref_token is None
        ):
            reg = GeoReferent()
        ...
        if (
            (ait.typ == AddressItemType.COUNTRY or ait.typ == AddressItemType.REGION)
        ) and (isinstance(ait.referent, GeoReferent)):
    ...
    def process_referent(self, begin: "Token", param: str) -> "ReferentToken":
        ...
        if ad.level > 1:
            return None
        ad.level += 1
        toks = CityItemToken.M_CITY_ADJECTIVES.try_parse_all(
            begin, TerminParseAttr.FULLWORDSONLY
        )
        ad.level -= 1
        res1 = None
        if toks is not None:
            for tok in toks:
                ...
                res1 = ReferentToken._new1850(
                    city,
                    tok.begin_token,
                    tok.end_token,
                    tok.morph,
                    begin.kit.get_analyzer_data(self),
                )
                break
        if not begin.morph.class0_.is_adjective:
            te = Utils.asObjectOrNull(begin, TextToken)
            if (
                te.chars.is_all_upper
                and te.chars.is_cyrillic_letter
                and te.length_char == 2
            ) and te.get_morph_class_in_dictionary().is_undefined:
                abbr = te.term
                geo0 = None
                cou = 0
                ...
                if cou == 1 and res1 is None:
                    res1 = ReferentToken._new1851(geo0, begin, begin, ad)
            ad.level += 1
            tt0 = TerrItemToken.try_parse(begin, None, None)
            ad.level -= 1
            if (
                tt0 is not None
                and tt0.termin_item is not None
                and tt0.termin_item.canonic_text == "РАЙОН"
            ):
                ad.level += 1
                ...
                if (
                    tt1 is not None
                    and tt1.chars.is_capital_upper
                    and tt1.termin_item is None
                ) and tt1.onto_item is None:
                    li = list()
                    li.append(tt0)
                    li.append(tt1)
            ...
            if ctoks is not None:
                if (
                    (
                        len(ctoks) == 2
                        and ctoks[0].typ == CityItemToken.ItemType.NOUN
                        and ctoks[1].typ == CityItemToken.ItemType.PROPERNAME
                    )
                ) or (
                    (len(ctoks) == 1 and ctoks[0].typ == CityItemToken.ItemType.CITY)
                ):
                    if (
                        len(ctoks) == 1
                        and ctoks[0]
                        .begin_token.get_morph_class_in_dictionary()
                        .is_proper_surname
                    ):
                        kk = begin.kit.process_referent(
                            "PERSON", ctoks[0].begin_token, None
                        )
                        if kk is not None:
                            return None
            ...
            if (
                ...
                and len(ctoks) == 1
                and ctoks[0].typ == CityItemToken.ItemType.NOUN
            ) and ctoks[0].value == "ГОРОД":
                cou = 0
                t = begin.previous
                first_pass3986 = True
                while True:
                    if first_pass3986:
                        first_pass3986 = False
                    else:
                        t = t.previous
                    if not (t is not None):
                        break
                    ...
                    for g in geos:
                        ...
                        if gg is not None:
                            res = None
                            if gg.is_city:
                                res = ReferentToken._new1850(
                                    gg, begin, ctoks[0].end_token, ctoks[0].morph, ad
                                )
                            ...
                            if res is not None and (
                                (res1 is None or res.length_char > res1.length_char)
                            ):
                                res1 = res
        ...
        if tt is None or tt.onto_item is None:
            ...
            if (
                tok is not None
                and tok[0].item is not None
                and (isinstance(tok[0].item.referent, GeoReferent))
            ) and tok[0].item.referent.is_state:
                tt = TerrItemToken._new1734(
                    tok[0].begin_token, tok[0].end_token, tok[0].item
                )
        if tt is None:
            return res1
                ...
                if res.begin_token == res.end_token:
                    ...
                    if mc.is_adjective:
                        ...
                        if geo_.is_city or geo_.is_state:
                            ...
                            geo_.find_slot(
                                GeoReferent.ATTR_TYPE, "федеральный округ", True
                            )
                            is not None
                        ):
                            return None
    ...
    def process_citizen(self, begin: "Token") -> "ReferentToken":
        ...
        if not (isinstance(begin, TextToken)):
            return None
        tok = TerrItemToken.M_MANS_BY_STATE.try_parse(
            begin, TerminParseAttr.FULLWORDSONLY
        )
        if tok is not None:
        ...
        res.morph = tok.morph
        ad = Utils.asObjectOrNull(
            begin.kit.get_analyzer_data(self), AnalyzerDataWithOntology
        )
        res.data = ad
        return res
