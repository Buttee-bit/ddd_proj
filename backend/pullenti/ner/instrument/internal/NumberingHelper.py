...
class NumberingHelper:
        ...
        if ((len(prev.numbers) - 1) == len(next0_.numbers) and len(prev.numbers) > 1): 
            ...
            while i < (len(prev.numbers) - 2): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12150 = RefOutArgWrapper(0)
            inoutres2151 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 2], wrapn12150)
            n1 = wrapn12150.value
            if (not inoutres2151): 
        ...
        if ((len(prev.numbers) - 2) == len(next0_.numbers) and len(prev.numbers) > 2): 
            ...
            while i < (len(prev.numbers) - 3): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12152 = RefOutArgWrapper(0)
            inoutres2153 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 3], wrapn12152)
            n1 = wrapn12152.value
            if (not inoutres2153): 
                return 0
            if ((n1 + 1) != n2): 
        ...
        if ((len(prev.numbers) - 3) == len(next0_.numbers) and len(prev.numbers) > 3): 
            ...
            while i < (len(prev.numbers) - 4): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12154 = RefOutArgWrapper(0)
            inoutres2155 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 4], wrapn12154)
            n1 = wrapn12154.value
            if (not inoutres2155): 
                return 0
            if ((n1 + 1) != n2): 
    ...
    def create_number(owner : 'FragToken', itok : 'InstrToken1') -> None:
        if ((itok.num_begin_token is None or itok.num_end_token is None or itok.numbers is None) or len(itok.numbers) == 0): 
            return
        num = FragToken._new2156(itok.num_begin_token, itok.num_end_token, InstrumentKind.NUMBER, True, itok)
    ...
    def create_diap(s1 : str, s2 : str) -> typing.List[str]:
        ...
        if (s2.startswith(s1)): 
            ...
            if (((i + 1) < len(s2)) and s2[i] == '.' and str.isdigit(s2[i + 1])): 
                wrapn22157 = RefOutArgWrapper(0)
                inoutres2158 = Utils.tryParseInt(s2[i + 1:], wrapn22157)
                n2 = wrapn22157.value
                if (inoutres2158): 
                    res0 = list()
                    res0.append(s1)
        ...
        if (((i)) > 0): 
            pref = s1[0:0+i + 1]
            wrapn12161 = RefOutArgWrapper(0)
            inoutres2162 = Utils.tryParseInt(s1[i + 1:], wrapn12161)
            n1 = wrapn12161.value
            if (not inoutres2162): 
                return None
            if (not s2.startswith(pref)): 
                return None
            wrapn22159 = RefOutArgWrapper(0)
            inoutres2160 = Utils.tryParseInt(s2[i + 1:], wrapn22159)
            n2 = wrapn22159.value
            if (not inoutres2160): 
                return None
        else: 
            wrapn12165 = RefOutArgWrapper(0)
            inoutres2166 = Utils.tryParseInt(s1, wrapn12165)
            n1 = wrapn12165.value
            if (not inoutres2166): 
                return None
            wrapn22163 = RefOutArgWrapper(0)
            inoutres2164 = Utils.tryParseInt(s2, wrapn22163)
            n2 = wrapn22163.value
            if (not inoutres2164): 
                return None
        if (n2 <= n1): 
