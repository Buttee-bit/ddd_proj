...
class ProcessorService:
            ...
            if (img is not None): 
                for kp in img.items(): 
                    if (not kp[0] in ProcessorService.__m_images): 
                        ProcessorService.__m_images[kp[0]] = ImageWrapper._new3595(kp[0], kp[1])
    ...
    def get_image_by_id(image_id : str) -> 'ImageWrapper':
        ...
        if (image_id is not None): 
            res = None
            wrapres3596 = RefOutArgWrapper(None)
            inoutres3597 = Utils.tryGetValue(ProcessorService.__m_images, image_id, wrapres3596)
            res = wrapres3596.value
            if (inoutres3597): 
                return res
    ...
    def add_image(image_id : str, content : bytearray) -> None:
        ...
        if (image_id is None): 
            return
        wr = ImageWrapper._new3595(image_id, content)
