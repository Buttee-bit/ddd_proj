...
class BookLinkAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (re.number is not None): 
                li = [ ]
                wrapli507 = RefOutArgWrapper(None)
                inoutres508 = Utils.tryGetValue(refs_by_num, re.number, wrapli507)
                li = wrapli507.value
                if (not inoutres508): 
                    li = list()
        ...
        while True:
            ...
            li = [ ]
            wrapli509 = RefOutArgWrapper(None)
            inoutres510 = Utils.tryGetValue(refs_by_num, Utils.ifNotNull(re.number, ""), wrapli509)
            li = wrapli509.value
            if (not inoutres510): 
                continue
            i = 0
    ...
    def __try_parse(t : 'Token', is_in_lit : bool, max_char : int=0) -> typing.List['ReferentToken']:
        ...
        next_num = None
        nn = 0
        wrapnn515 = RefOutArgWrapper(0)
        inoutres516 = Utils.tryParseInt(Utils.ifNotNull(num, ""), wrapnn515)
        nn = wrapnn515.value
        if (inoutres516): 
            next_num = str((nn + 1))
        ...
        if (tn1 is None and start_of_name is None): 
            if (is_electr_res): 
                ...
                rts0 = list()
                bref0 = BookLinkRefReferent._new511(uri_re)
                if (num is not None): 
                    bref0.number = num
            ...
            if (decree is not None and num is not None): 
                rts0 = list()
                bref0 = BookLinkRefReferent._new511(decree.get_referent())
                if (num is not None): 
                    bref0.number = num
            ...
            if (book_prev is not None): 
                ...
                if (blt0 is not None and blt0.typ == BookLinkTyp.PAGERANGE): 
                    rts0 = list()
                    bref0 = BookLinkRefReferent._new511(book_prev)
                    if (num is not None): 
                        bref0.number = num
        ...
        rts = list()
        bref = BookLinkRefReferent._new511(res)
        if (num is not None): 
            bref.number = num
