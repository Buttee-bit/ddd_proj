...
from backend.pullenti.unisharp.Misc import CancelEventArgs
...
from backend.pullenti.ner.core.internal.ProgressPeace import ProgressPeace
...
class Processor(object):
    ...
    def find_analyzer(self, name : str) -> 'Analyzer':
        ...
        a = None
        with self.__m_lock: 
            wrapa3587 = RefOutArgWrapper(None)
            inoutres3588 = Utils.tryGetValue(self.__m_analyzers_hash, Utils.ifNotNull(name, ""), wrapa3587)
            a = wrapa3587.value
            if (inoutres3588): 
                return a
    ...
    def process_next(self, ar : 'AnalysisResult') -> None:
        ...
        if (ar is None): 
            return
        kit = AnalysisKit._new3589(self, ar.ontology)
        kit._init_from(ar)
    ...
    def _process(self, text : 'SourceOfAnalysis', onto_regine : bool, no_log : bool, ext_ontology : 'ExtOntology'=None, lang : 'MorphLang'=None) -> 'AnalysisResult':
        ...
        kit = AnalysisKit._new3590(text, False, lang, self.__progress_changed_event_handler_on_progress_handler, ext_ontology, self, onto_regine)
        ar = AnalysisResult()
        sw0.stop()
        msg = None
    ...
    def __prepare_progress(self) -> None:
        with self.__m_progress_peaces_lock: 
            ...
            max0_ = co * 100
            max0_ /= (total)
            self.__m_progress_peaces[self] = ProgressPeace._new3591(0, max0_)
            for wf in self.analyzers: 
                if (not wf.ignore_this_analyzer): 
                    min0_ = max0_
                    ...
                    if (not wf in self.__m_progress_peaces): 
                        self.__m_progress_peaces[wf] = ProgressPeace._new3591(min0_, max0_)
    ...
    def _on_progress_handler(self, sender : object, e0_ : ProgressEventArgs) -> None:
        ...
        if (e0_.progressPercentage >= 0): 
            pi0_ = None
            with self.__m_progress_peaces_lock: 
                wrappi3593 = RefOutArgWrapper(None)
                inoutres3594 = Utils.tryGetValue(self.__m_progress_peaces, Utils.ifNotNull(sender, self), wrappi3593)
                pi0_ = wrappi3593.value
                if (inoutres3594): 
