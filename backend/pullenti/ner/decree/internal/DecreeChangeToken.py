...
class DecreeChangeToken(MetaToken):
                    ...
                    for np in tok.new_parts: 
                        ...
                        if (len(np.values) == 0): 
                            if (eq_lev_val is None): 
                                ...
                                n = 0
                                wrapn1021 = RefOutArgWrapper(0)
                                inoutres1022 = Utils.tryParseInt(eq_lev_val, wrapn1021)
                                n = wrapn1021.value
                                if (inoutres1022): 
    ...
    def __skip_decree_dummy(self) -> None:
        ...
        while True:
            ...
            if (li is not None and len(li) > 0): 
                ...
                continue
            if (tt.is_char('(')): 
                br = BracketHelper.try_parse(tt, BracketParseAttr.CANBEMANYLINES, 1000)
                if (br is not None): 
                    tt = br.end_token
                    self.end_token = tt
                    continue
    ...
    def try_attach(t : 'Token', main : 'DecreeChangeReferent'=None, ignore_newlines : bool=False, change_stack : typing.List['Referent']=None, is_in_edition : bool=False, abzac_regime : bool=False) -> 'DecreeChangeToken':
        ...
        if ((isinstance(tt, TextToken)) and t.is_newline_before and not ignore_newlines): 
            if (tt.is_value("ВНЕСТИ", "УНЕСТИ") and ((((tt.next0_ is not None and tt.next0_.is_value("В", "ДО"))) or tt.term == "ВНЕСТИ" or tt.term == "УНЕСТИ"))): 
                ...
                while True:
                    ...
                    break
        if (te1 is not None): 
            res = DecreeChangeToken._new1023(tt, te1, DecreeChangeTokenTyp.STARTMULTU)
            tt = te1.next0_
            first_pass3864 = True
            while True:
        ...
        if (tt_expire is not None): 
            res = DecreeChangeToken._new1024(tt, tt_expire, DecreeChangeTokenTyp.ACTION, (DecreeChangeKind.SUSPEND if is_suspend else DecreeChangeKind.EXPIRE))
            day_after = False
            tt = tt_expire.next0_
            first_pass3866 = True
            while True:
        ...
        if ((not ignore_newlines and ((is_newline_before_ or tt == t)) and tt.is_value("УТРАТИТЬ", "ВТРАТИТИ")) and tt.next0_ is not None and tt.next0_.is_value("СИЛА", "ЧИННІСТЬ")): 
            res = DecreeChangeToken._new1023(tt, tt.next0_, DecreeChangeTokenTyp.UNDEFINED)
            tt = tt.next0_
        ...
        if (not ignore_newlines and is_newline_before_ and not tt.is_value2("В", "КОНЦЕ")): 
            if (tt.is_value("СЛОВО", None)): 
                pass
            res = DecreeChangeToken._new1023(tt, tt, DecreeChangeTokenTyp.STARTSINGLE)
            izlogit = None
            spec_regime = False
            first_pass3867 = True
            while True:
                ...
                if (li is None and tt.is_value("ПРИМЕЧАНИЕ", "ПРИМІТКА")): 
                    li = list()
                    li.append(PartToken._new1027(tt, tt, PartToken.ItemType.NOTICE))
                        ...
                        if (res.parts is not None): 
                            ...
                            while ttt1 is not None: 
                                ...
                                if (ttt1.is_value3("ВСТУПАТЬ", "В", "СИЛУ")): 
                                    res.ignorable = True
                                    break
                                if (ttt1.is_value3("ИЗЛОЖИТЬ", "В", "СЛЕДУЮЩЕЙ")): 
                                    izlogit = ttt1.previous
                                if (ttt1.is_value3("ИЗЛОЖИТЬ", "В", "НОВОЙ")): 
                                    izlogit = ttt1.previous
                                ttt1 = ttt1.next0_
                ...
                if (tt == res.begin_token and main is not None): 
                    ...
                    if (npt is not None): 
                        ...
                        if ((tt1 is not None and tt1.is_value("ИЗЛОЖИТЬ", "ВИКЛАСТИ") and tt1.next0_ is not None) and tt1.next0_.is_value("В", None)): 
                            pt = PartToken._new1027(tt, npt.end_token, PartToken.ItemType.APPENDIX)
                ...
                if (tt.next0_ is not None): 
                    ...
                    if (tt2 is not None and tt2.is_value2("В", "РЕДАКЦИИ")): 
                        tt2 = tt2.next0_.next0_
                    if (tt2 is not None and tt2.is_value2("СОГЛАСНО", "ПРИЛОЖЕНИЮ")): 
        ...
        if (tok is None and tt.is_value("ПОСЛЕ", None) and BracketHelper.can_be_start_of_sequence(tt.next0_, True, False)): 
            ...
            tok.termin = Termin._new89("СЛОВО", DecreeChangeValueKind.WORDS)
            after_value = True
        if (tok is not None): 
            if (isinstance(tok.termin.tag, DecreeChangeKind)): 
                res = DecreeChangeToken._new1024(tt, tok.end_token, DecreeChangeTokenTyp.ACTION, Utils.valToEnum(tok.termin.tag, DecreeChangeKind))
            ...
            if (isinstance(tok.termin.tag, DecreeChangeValueKind)): 
                res = DecreeChangeToken._new1023(tt, tok.end_token, DecreeChangeTokenTyp.VALUE)
                res.change_val = DecreeChangeValueReferent()
        ...
        if (((is_nex_change > 0 and ((BracketHelper.is_bracket(xx1, True) or abzac_regime or ComplexNumToken.try_parse(xx1, None, False, False) is not None)))) or ((is_nex_change > 1 and t.is_value("ПРИЛОЖЕНИЕ", "ДОДАТОК")))): 
            if (is_in_edition): 
                res = DecreeChangeToken._new1023(t, xx1, DecreeChangeTokenTyp.VALUE)
        ...
        while True:
            ...
            if (not tt.is_newline_after): 
                ...
                if (nnn is not None): 
                    ...
                    seq_num = 0
                    nnn0 = None
                    wrapnnn01036 = RefOutArgWrapper(None)
                    inoutres1037 = Utils.tryGetValue(last_nums, Utils.ifNotNull(nnn.suffix, ""), wrapnnn01036)
                    nnn0 = wrapnnn01036.value
                    if (inoutres1037): 
            ...
            if ((part is not None and len(part.values) == 1 and tt.is_newline_after) and part.morph.case_.is_nominative and not tt.next0_.chars.is_all_lower): 
                if (tt != tt.next0_ and len(pstack) == 0): 
                    ...
                    n0 = 0
                    n1 = 0
                    wrapn01038 = RefOutArgWrapper(0)
                    inoutres1039 = Utils.tryParseInt(pstack[part.typ], wrapn01038)
                    wrapn11040 = RefOutArgWrapper(0)
                    inoutres1041 = Utils.tryParseInt(part.values[0].value, wrapn11040)
                    n0 = wrapn01038.value
                    n1 = wrapn11040.value
                    if (inoutres1039 and inoutres1041): 
                        if ((n0 + 1) == n1): 
                            doubt1 = (None)
                    pstack[part.typ] = part.values[0].value
    ...
    def __try_attach_list(t : 'Token', ignore_newline : bool=False, abzac_regime : bool=False) -> typing.List['DecreeChangeToken']:
        ...
        while True:
            ...
            if (d is None): 
                ...
                if (pts is not None): 
                    d = DecreeChangeToken._new1023(pts[0].begin_token, pts[len(pts) - 1].end_token, DecreeChangeTokenTyp.UNDEFINED)
            ...
            if (d.indent_regime): 
                ...
                abzac_regime = False
        if ((len(res) == 1 and res[0].act_kind == DecreeChangeKind.CONSIDER and res[0].end_token.next0_ is not None) and res[0].end_token.next0_.is_comma_and): 
    ...
    def _initialize() -> None:
        ...
        t = None
        t = Termin._new89("ИЗЛОЖИТЬ В СЛЕДУЮЩЕЙ РЕДАКЦИИ", DecreeChangeKind.NEW)
        ...
        t = Termin._new496("ВИКЛАСТИ В НАСТУПНІЙ РЕДАКЦІЇ", MorphLang.UA, DecreeChangeKind.NEW)
        ...
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("ПРИЗНАТЬ УТРАТИВШИМ СИЛУ", DecreeChangeKind.EXPIRE)
        t.add_variant("СЧИТАТЬ УТРАТИВШИМ СИЛУ", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new496("ВИЗНАТИ таким, що ВТРАТИВ ЧИННІСТЬ", MorphLang.UA, DecreeChangeKind.EXPIRE)
        t.add_variant("ВВАЖАТИ таким, що ВТРАТИВ ЧИННІСТЬ", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("ИСКЛЮЧИТЬ", DecreeChangeKind.REMOVE)
        t.add_variant("ИСКЛЮЧИВ ИЗ НЕГО", False)
        t.add_variant("УДАЛИТЬ", False)
        t.add_variant("УДАЛИВ ИЗ НЕГО", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new496("ВИКЛЮЧИТИ", MorphLang.UA, DecreeChangeKind.REMOVE)
        t.add_variant("ВИКЛЮЧИВШИ З НЬОГО", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("ПРИОСТАНОВИТЬ ДЕЙСТВИЕ", DecreeChangeKind.SUSPEND)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("СЧИТАТЬ", DecreeChangeKind.CONSIDER)
        t.add_variant("СЧИТАТЬ СООТВЕТСТВЕННО", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new496("ВВАЖАТИ", MorphLang.UA, DecreeChangeKind.CONSIDER)
        t.add_variant("ВВАЖАТИ ВІДПОВІДНО", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("ЗАМЕНИТЬ", DecreeChangeKind.EXCHANGE)
        t.add_variant("ЗАМЕНИВ В НЕМ", False)
        t.add_variant("ИСКЛЮЧИТЬ ЗАМЕНИТЬ", False)
        t.add_variant("ИСКЛЮЧИТЬ И ЗАМЕНИТЬ", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new496("ЗАМІНИТИ", MorphLang.UA, DecreeChangeKind.EXCHANGE)
        t.add_variant("ЗАМІНИВШИ В НЬОМУ", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("ДОПОЛНИТЬ", DecreeChangeKind.APPEND)
        t.add_variant("ДОПОЛНИВ ЕГО", False)
        t.add_variant("ДОБАВИТЬ", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new496("ДОПОВНИТИ", MorphLang.UA, DecreeChangeKind.APPEND)
        t.add_variant("ДОПОВНИВШИ ЙОГО", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("СЛОВО", DecreeChangeValueKind.WORDS)
        ...
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new91("ОПРЕДЕЛЕНИЕ", DecreeChangeValueKind.DEFINITION, ";")
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new91("ТОЧКА С ЗАПЯТОЙ", DecreeChangeValueKind.WORDS, ";")
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new91("ТОЧКА", DecreeChangeValueKind.WORDS, ".")
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new91("ВОПРОСИТЕЛЬНЫЙ ЗНАК", DecreeChangeValueKind.WORDS, "?")
        t.add_variant("ЗНАК ВОПРОСА", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new91("ВОСКЛИЦАТЕЛЬНЫЙ ЗНАК", DecreeChangeValueKind.WORDS, "!")
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("ЦИФРА", DecreeChangeValueKind.NUMBERS)
        t.add_variant("ЧИСЛО", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("БЛОК", DecreeChangeValueKind.BLOCK)
        t.add_variant("БЛОК СО СЛОВАМИ", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new496("БЛОК", MorphLang.UA, DecreeChangeValueKind.BLOCK)
        t.add_variant("БЛОК ЗІ СЛОВАМИ", False)
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new89("В СООТВЕТСТВУЮЩИХ ЧИСЛЕ И ПАДЕЖЕ", DecreeChangeValueKind.ROBUSTWORDS)
        ...
        DecreeChangeToken.M_TERMS.add(t)
        t = Termin._new496("У ВІДПОВІДНОМУ ЧИСЛІ ТА ВІДМІНКУ", MorphLang.UA, DecreeChangeValueKind.ROBUSTWORDS)
        t.add_variant("У ВІДПОВІДНОМУ ВІДМІНКУ", False)
        t.add_variant("У ВІДПОВІДНОМУ ЧИСЛІ", False)
        DecreeChangeToken.M_TERMS.add(t)
    
    @staticmethod
    def _new1023(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeChangeTokenTyp') -> 'DecreeChangeToken':
        res = DecreeChangeToken(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new1024(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeChangeTokenTyp', _arg4 : 'DecreeChangeKind') -> 'DecreeChangeToken':
        res = DecreeChangeToken(_arg1, _arg2)
        res.typ = _arg3
        res.act_kind = _arg4
        return res