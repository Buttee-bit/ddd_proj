...
class backend.pullentiNerDecreeInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('backend.pullenti.ner.booklink.properties', '.png')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('backend.pullenti.ner.booklink.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('backend.pullenti.ner.booklink.properties', n) as stream: 
    ...
    def get_string(name : str) -> str:
        arr = backend.pullentiNerDecreeInternalResourceHelper.get_bytes(name)
        if (arr is None): 
            return None
