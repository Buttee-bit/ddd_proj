...
class DecreeAnalyzer(Analyzer):
    ...
    class ThisDecree(MetaToken):
        ...
        def try_attach_back(t : 'Token', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (ukaz is not None): 
                if (base_typ is not None and base_typ.value is not None and (("ДОГОВОР" in base_typ.value or "ДОГОВІР" in base_typ.value))): 
                    return DecreeAnalyzer.ThisDecree._new1395(ukaz, ukaz, True, base_typ.value)
            return None
        ...
        def try_attach(dtok : 'PartToken', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (not (isinstance(tt, TextToken))): 
                if ((isinstance(tt, ReferentToken)) and (isinstance(tt.get_referent(), DecreeReferent))): 
                    return DecreeAnalyzer.ThisDecree._new1396(t, tt, Utils.asObjectOrNull(tt.get_referent(), DecreeReferent))
                return None
        ...
        @staticmethod
        def _new1395(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : str) -> 'ThisDecree':
        ...
        @staticmethod
        def _new1396(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeReferent') -> 'ThisDecree':
            res = DecreeAnalyzer.ThisDecree(_arg1, _arg2)
            res.real = _arg3
            return res
            ...
            if (isinstance(tt, TextToken)): 
                if (tt.is_pure_verb): 
                    if (tt.previous.is_value("КОТОРЫЙ", None)): 
                        no_verb = True
                    elif (tt.morph.contains_attr("к.ф.", None) or tt.morph.contains_attr("инф.", None)): 
                        no_verb = True
    ...
    def _try_attach_approved0(t : 'Token', ad : 'AnalyzerData', must_be_comma : bool=True, name_in_brackets : bool=False) -> 'ReferentToken':
        ...
        olev = None
        lev = 0
        wrapolev1381 = RefOutArgWrapper(None)
        inoutres1382 = Utils.tryGetValue(kit.misc_data, "dovr", wrapolev1381)
        olev = wrapolev1381.value
        if (not inoutres1382): 
            lev = 1
    ...
    def __try_attach_pulishers(self, dts : typing.List['DecreeToken']) -> typing.List['ReferentToken']:
        ...
        while True:
            ...
            if (pt is not None): 
                ...
                if (pub_part is None): 
                    pub_part = DecreePartReferent._new1383(pub)
            ...
            if (isinstance(t, NumberToken)): 
                ...
                if (rt is not None): 
                    date_ = DecreeToken._new1082(rt.begin_token, rt.end_token, DecreeToken.ItemType.DATE)
                    date_.ref = rt
                    pub0 = (None)
                    pub_part0 = (None)
                    if (t0 is None): 
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if ((len(dts) == 1 and dts[0].typ == DecreeToken.ItemType.TYP and dts[0].value != "КОНСТИТУЦИЯ") and not dts[0].typ_ref_back): 
                continue
            if ((len(dts) < 3) and dts[0].value == "РЕГЛАМЕНТ"): 
                continue
            ...
            if (rts is not None): 
                for kp in rts: 
                    ...
                    while True:
                        ...
                        rt = None
                        if (i == 0): 
                            rt = ReferentToken._new939(dr, kp.begin_token, kp.end_token, kp.morph)
                        else: 
            ...
            if (dcht is not None and ((has_start_change > 0 or expire_regime > 0)) and not dcht.ignorable): 
                ...
                if (dcht.typ == DecreeChangeTokenTyp.STARTMULTU and ((root_change_ques is None or root_change_ques.decree != dcht.decree))): 
                    ...
                    if (((len(change_stack) > 0 or dcht.decree is not None)) and expire_regime == 0): 
                        rch = Utils.asObjectOrNull(ad.register_referent(DecreeChangeReferent._new1387(DecreeChangeKind.CONTAINER)), DecreeChangeReferent)
                ...
                if (dprs is not None and len(dprs) > 0): 
                    ...
                    if (chrt is None and expire_regime > 0): 
                        chrt = list()
                        dcr = DecreeChangeReferent._new1387((DecreeChangeKind.EXPIRE if expire_regime == 1 else DecreeChangeKind.SUSPEND))
                ...
                if (chrt is None and ((expire_regime > 0 or dcht.act_kind == DecreeChangeKind.EXPIRE or dcht.act_kind == DecreeChangeKind.SUSPEND)) and dprs is None): 
                    chrt = list()
                    if (dcht.decree is not None): 
                        dcr = DecreeChangeReferent._new1387(DecreeChangeKind.EXPIRE)
                    ...
                    while True:
                        ...
                        if (isinstance(tt.get_referent(), DecreeReferent)): 
                            dcr = DecreeChangeReferent._new1387((DecreeChangeKind.EXPIRE if expire_regime == 1 or dcht.act_kind == DecreeChangeKind.EXPIRE else DecreeChangeKind.SUSPEND))
    ...
    def _check_alias_after(t : 'Token') -> 'MetaToken':
        ...
        if ((isinstance(t.get_referent(), DecreeReferent)) and t.next0_ is not None and t.next0_.is_char(')')): 
            return MetaToken._new648(t, t.next0_, t.get_referent())
        t1 = None
        tt = t
        while tt is not None: 
            if (tt.is_newline_before): 
    ...
    def _try_attach_parts(parts : typing.List['PartToken'], base_typ : 'DecreeToken', _def_owner : 'Referent', in_changes : bool=False) -> typing.List['MetaToken']:
        ...
        while True:
            ...
            j = (i + 1)
            while j < len(parts): 
                if ((len(parts[j].values) == 0 and parts[j].typ != PartToken.ItemType.PREAMBLE and parts[j].typ != PartToken.ItemType.NOTICE) and parts[j].typ != PartToken.ItemType.TABLE and parts[j].typ != PartToken.ItemType.FOOTNOTE): 
                    break
            ...
            j = (i + 1)
            while j < len(parts): 
                if (((len(parts[j].values) == 0 and parts[j].name is None and parts[j].typ != PartToken.ItemType.PREAMBLE) and parts[j].typ != PartToken.ItemType.NOTICE and parts[j].typ != PartToken.ItemType.FOOTNOTE) and parts[j].typ != PartToken.ItemType.TABLE): 
                    break
        ...
        while i < len(res): 
            ...
            while et.begin_char > res[i].begin_char:
                if (et.is_char(',') or et.morph.class0_.is_conjunction or et.is_hiphen): 
                    ...
                    break
            res1.append(MetaToken._new1394(res[i].begin_token, et, li, res[i].morph))
            i = (j - 1)
            i += 1
        return res1