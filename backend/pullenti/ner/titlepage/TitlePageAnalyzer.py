...
class TitlePageAnalyzer(Analyzer):
    ...
    def process_referent1(self, begin : 'Token', end : 'Token') -> 'ReferentToken':
        et = None
        wrapet3406 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(begin, (0 if end is None else end.end_char), begin.kit, wrapet3406)
        et = wrapet3406.value
        if (tpr is None): 
            return None
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        et = None
        wrapet3407 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(kit.first_token, 0, kit, wrapet3407)
        et = wrapet3407.value
        if (tpr is not None): 
            ...
            for r in rli: 
                ...
                if (isinstance(r, OrganizationReferent)): 
                    ...
                    if ("курс" in org0_.types and org0_.number is not None): 
                        i = 0
                        wrapi3408 = RefOutArgWrapper(0)
                        inoutres3409 = Utils.tryParseInt(org0_.number, wrapi3408)
                        i = wrapi3408.value
                        if (inoutres3409): 
                            if (i > 0 and (i < 8)): 
                                res.student_year = i
