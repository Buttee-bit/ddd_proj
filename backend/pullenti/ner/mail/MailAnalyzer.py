...
from backend.pullenti.ner.person.internal.PullentiNerPersonInternalResourceHelper import (
    PullentiNerPersonInternalResourceHelper,
)
from backend.pullenti.ner.mail.internal.MetaLetter import MetaLetter
from backend.pullenti.ner.mail.MailKind import MailKind
...
class MailAnalyzer(Analyzer):
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MetaLetter.IMAGE_ID] = PullentiNerPersonInternalResourceHelper.get_bytes(
            "mail.png"
        )
        return res
        ...
        while True:
            ...
            if ml.typ == MailLine.Types.FROM:
                ...
                if ((i + 2) < len(lines)) and (
                    (
                        (
                            lines[i + 1].typ == MailLine.Types.FROM
                            or lines[i + 2].typ == MailLine.Types.FROM
                            or lines[i + 1].typ == MailLine.Types.HELLO
                        )
                        or lines[i + 2].typ == MailLine.Types.HELLO
                    )
                ):
                    is_new = True
                if not is_new:
                ...
                if not is_new:
                    ...
                    while tt is not None and tt.end_char <= ml.end_char:
                        if tt.get_referent() is not None:
                            if (
                                tt.get_referent().type_name == "DATE"
                                or tt.get_referent().type_name == "URI"
                            ):
                                is_new = True
                        tt = tt.next0_
                if is_new:
                    ...
                    while True:
                        ...
                        if lines[i].typ == MailLine.Types.FROM:
                            if len(blk) > 0 and lines[i].must_be_first_line:
                                break
                            blk.append(lines[i])
                        elif ((i + 1) < len(lines)) and lines[
                            i + 1
                        ].typ == MailLine.Types.FROM:
                            j = 0
                            j = 0
                            while j < len(blk):
                                if blk[j].typ == MailLine.Types.FROM:
                                    if (
                                        blk[j].is_real_from
                                        or blk[j].must_be_first_line
                                        or blk[j].mail_addr is not None
                                    ):
                                        break
                                j += 1
                            if j >= len(blk):
        ...
        while True:
            ...
            if lines[0].typ == MailLine.Types.FROM:
                ...
                while i < len(lines):
                    if lines[i].typ == MailLine.Types.FROM:
                        ...
                        break
                    i += 1
                mail_ = MailReferent._new2199(MailKind.HEAD)
                mt = ReferentToken(mail_, lines[0].begin_token, t1)
                mail_.text = MiscHelper.get_text_value_of_meta_token(
                    mt, GetTextAttr.KEEPREGISTER
                )
            ...
            while True:
                ...
                if li.typ == MailLine.Types.BESTREGARDS:
                    ...
                    i -= 1
                    while i >= i0:
                        if lines[i].typ == MailLine.Types.BESTREGARDS and (
                            lines[i].words < 2
                        ):
                            t2 = lines[i].begin_token
                        elif (
                            i > i0
                            and (lines[i].words < 3)
                            and lines[i - 1].typ == MailLine.Types.BESTREGARDS
                        ) and (lines[i - 1].words < 2):
                            i -= 1
                            t2 = lines[i].begin_token
                        else:
                            break
            ...
            if t2 is None:
                for i in range(len(lines) - 1, i0 - 1, -1):
                    ...
                    if li.typ == MailLine.Types.UNDEFINED:
                        if len(li.refs) > 0 and (
                            isinstance(li.refs[0], PersonReferent)
                        ):
            ...
            while ii < len(lines):
                if lines[ii].typ == MailLine.Types.HELLO:
                    mail_ = MailReferent._new2199(MailKind.HELLO)
                    mt = ReferentToken(
                        mail_, lines[i0].begin_token, lines[ii].end_token
                    )
                    if mt.length_char > 0:
                        mail_.text = MiscHelper.get_text_value_of_meta_token(
                            mt, GetTextAttr.KEEPREGISTER
                    ...
                    lines[ii].typ != MailLine.Types.UNDEFINED
                    or lines[ii].words > 0
                    or len(lines[ii].refs) > 0
                ):
                    break
                ii += 1
            if i0 < len(lines):
                if t2 is not None and t2.previous is None:
                    ...
                    tt11 = (
                        t2.previous
                        if t2 is not None and t2.previous is not None
                        else lines[len(lines) - 1].end_token
                    )
                    if tt00.begin_char > tt11.end_char:
                        pass
                    else:
                        mail_ = MailReferent._new2199(MailKind.BODY)
                        ...
                        if mt.length_char > 0:
                            ...
                            mail_.add_occurence_of_ref_tok(mt)
                if t2 is not None:
                    mail_ = MailReferent._new2199(MailKind.TAIL)
