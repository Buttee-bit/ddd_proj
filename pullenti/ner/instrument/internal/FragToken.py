...
class FragToken(MetaToken):
    ...
    def _create_referent(self, ad : 'AnalyzerData', bas : 'FragToken') -> 'InstrumentBlockReferent':
        ...
        if (self._m_doc is not None): 
            ...
            if (self.kind == InstrumentKind.TABLE): 
                if (self.table_columns > 0): 
                    res.table_columns = self.table_columns
            if (self.is_expired): 
                res.is_expired = True
            if (self.is_error): 
                res.is_error = True
    ...
    def fill_by_content_children(self) -> None:
        self.sort_children()
        if (len(self.children) == 0): 
            self.children.append(FragToken._new1945(self.begin_token, self.end_token, InstrumentKind.CONTENT))
            return
        if (self.begin_char < self.children[0].begin_char): 
            self.children.insert(0, FragToken._new1945(self.begin_token, self.children[0].begin_token.previous, InstrumentKind.CONTENT))
        i = 0
        while i < (len(self.children) - 1): 
            if (self.children[i].end_token.next0_ != self.children[i + 1].begin_token and (self.children[i].end_token.next0_.end_char < self.children[i + 1].begin_char)): 
                self.children.insert(i + 1, FragToken._new1945(self.children[i].end_token.next0_, self.children[i + 1].begin_token.previous, InstrumentKind.CONTENT))
            i += 1
        if (self.children[len(self.children) - 1].end_char < self.end_char): 
            self.children.append(FragToken._new1945(self.children[len(self.children) - 1].end_token.next0_, self.end_token, InstrumentKind.CONTENT))
    ...
    def create_document(t : 'Token', max_char : int, root_kind : 'InstrumentKind'=InstrumentKind.UNDEFINED) -> 'FragToken':
        ...
        if (res is not None): 
            return res
        res = FragToken._new1945(t, t, InstrumentKind.DOCUMENT)
            ...
            if (li[i].typ == ILTypes.APPENDIX or li[i].typ == ILTypes.APPROVED): 
                ...
                if (isinstance(li[i].ref, DecreeReferent)): 
                    ...
                    while True:
                        ...
                        else: tt = tt.next0_; cou2 -= 1
                        if (not (tt is not None and cou2 > 0)): break
                        dr = Utils.asObjectOrNull(tt.get_referent(), DecreeReferent)
                        if (dr is None): 
                            continue
                        if (dr.typ != res._m_doc.typ): 
                            continue
                        if (dr.number is not None and res._m_doc.reg_number is not None): 
                            if (dr.number != res._m_doc.reg_number): 
                                continue
                        ok = True
                        break
                    if (not ok): 
        ...
        for i in range(i1 - 1, 0, -1):
            ...
            if ((lii.typ == ILTypes.PERSON or lii.typ == ILTypes.REGNUMBER or lii.typ == ILTypes.DATE) or lii.typ == ILTypes.GEO): 
                ...
                if (not lii.is_newline_before and not lii.begin_token.is_table_control_char and ((lii.typ == ILTypes.GEO or li[i].typ == ILTypes.PERSON))): 
                    ...
                    if (not lii.is_newline_after): 
                        if (lii.end_token.next0_ is not None and not lii.end_token.next0_.is_table_control_char): 
                            break
                if (tail is None): 
                    tail = FragToken._new1945(li[i].begin_token, li[i1 - 1].end_token, InstrumentKind.TAIL)
                    if ((i1 - 1) > i): 
                        pass
                ...
                if (li[i].is_podpis_storon and tail is not None): 
                    ...
                    tail.children.insert(0, FragToken._new2007(li[i].begin_token, li[i].end_token, InstrumentKind.NAME, ss))
                    cmax = (i - 1)
                    break
                jj = 0
                jj = 0
        ...
        if (cmax < 0): 
            ...
            content = FragToken._new1945(li[0].begin_token, li[cmax].end_token, InstrumentKind.CONTENT)
        ...
        if (tail is not None): 
            ...
            while i1 < len(li): 
                if (li[i1].begin_token == li[i1].end_token and (isinstance(li[i1].begin_token.get_referent(), DecreeReferent)) and li[i1].begin_token.get_referent().kind == DecreeKind.PUBLISHER): 
                    ap = FragToken._new1945(li[i1].begin_token, li[i1].end_token, InstrumentKind.APPROVED)
                    ap.referents = list()
            ...
            if (len(tail.children) > 0 and (tail.children[len(tail.children) - 1].end_char < tail.end_char)): 
                unkw = FragToken._new1945(tail.children[len(tail.children) - 1].end_token.next0_, tail.end_token, InstrumentKind.UNDEFINED)
        ...
        while i < len(li): 
            ...
            if (li[j - 1].typ == ILTypes.PERSON and li[j - 1].is_newline_before and li[j - 1].is_newline_after): 
                tail = FragToken._new1945(li[j - 1].begin_token, li[j - 1].end_token, InstrumentKind.TAIL)
                for jj in range(j - 1, i, -1):
                    if (li[jj].typ != ILTypes.PERSON or not li[jj].is_newline_before or not li[jj].is_newline_after): 
                        break
                    else: 
                        fr = FragToken._new1945(li[jj].begin_token, li[jj].end_token, InstrumentKind.SIGNER)
            ...
            if (title is None): 
                ...
                if (is_app and app.kind == InstrumentKind.APPENDIX): 
                    ...
                    res0 = FragToken._new2013(res.begin_token, res.end_token, res._m_doc, InstrumentKind.DOCUMENT)
                    res._m_doc = (None)
                ...
                if (title.end_token.next0_ is not None): 
                    if (title.end_token.end_char < app.end_token.begin_char): 
                        acontent = FragToken._new1945(title.end_token.next0_, app.end_token, InstrumentKind.CONTENT)
    ...
    def __create_case_info(t : 'Token') -> 'FragToken':
        ...
        if ((not has_date and t1.next0_ is not None and (isinstance(t1.next0_.get_referent(), DateReferent))) and t1.next0_.is_newline_after): 
            t1 = t1.next0_
        return FragToken._new1945(t, t1, InstrumentKind.CASEINFO)
    ...
    def __create_approved(t : 'Token') -> 'FragToken':
        ...
        if (((isinstance(t, ReferentToken)) and t.begin_token.is_char('(') and t.end_token.is_char(')')) and t.begin_token.next0_.is_value("ПРОТОКОЛ", None)): 
            res = FragToken._new1945(t, t, InstrumentKind.APPROVED)
            res.referents = list()
            res.referents.append(t.get_referent())
            return res
        tt = InstrToken._check_approved(t)
        if (tt is not None): 
            res = FragToken._new1945(t, tt, InstrumentKind.APPROVED)
        ...
        if (dts is not None and len(dts) > 0): 
            ...
            while i < len(dts): 
                ...
                if (dt.typ == DecreeToken.ItemType.ORG): 
                    res.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.ORGANIZATION, dt))
                elif (dt.typ == DecreeToken.ItemType.OWNER): 
                    res.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.INITIATOR, dt))
                elif (dt.typ == DecreeToken.ItemType.DATE): 
                    res.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.DATE, dt))
                elif (dt.typ == DecreeToken.ItemType.NUMBER and i > 0 and dts[i - 1].typ == DecreeToken.ItemType.DATE): 
                    res.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.NUMBER, dt))
                    ...
                    res.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.PLACE, dt))
                else: 
                    break
            ...
            while True:
                ...
                if (dr is None): 
                    break
                if (len(res.referents) > 0 and t.newlines_before_count > 3): 
                    break
    ...
    def _create_misc(t : 'Token') -> 'FragToken':
        ...
        if (t.is_value("ФОРМА", None) and t.next0_.is_value("ДОКУМЕНТА", None)): 
            ...
            if (num is not None and num.typ == DecreeToken.ItemType.NUMBER): 
                return FragToken._new1945(t, num.end_token, InstrumentKind.UNDEFINED)
            if ((isinstance(t.next0_.next0_, NumberToken)) and t.next0_.next0_.is_newline_after): 
                return FragToken._new1945(t, t.next0_.next0_, InstrumentKind.UNDEFINED)
        ...
        if (t.is_char('(')): 
            ...
            if (npt is not None and npt.end_token.next0_ is not None and npt.end_token.next0_.is_char(')')): 
                return FragToken._new1945(t, npt.end_token.next0_, InstrumentKind.UNDEFINED)
        if ((isinstance(t, NumberToken)) and t.next0_ is not None and t.next0_.is_value("СЕССИЯ", None)): 
            return FragToken._new1945(t, t.next0_, InstrumentKind.UNDEFINED)
        if (t.is_value("С", None) and t.next0_.is_value("ИЗМЕНЕНИЕ", None) and t.next0_.next0_ is not None): 
            ...
            if (isinstance(tt.get_referent(), DateReferent)): 
                if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                    tt = tt.next0_
                return FragToken._new1945(t, tt, InstrumentKind.UNDEFINED)
        t0 = t
        ...
        if (t.is_value("ЗАКАЗ", None)): 
            ...
            if (itt is not None): 
                return FragToken._new1945(t, itt.end_token, InstrumentKind.UNDEFINED)
        nums = 0
        spec = 0
        chars_ = 0
        words = 0
        ...
        while True:
            ...
            if ((nums + spec + chars_) > (words * 3)): 
                ...
                break
            return FragToken._new1945(t0, t1, InstrumentKind.UNDEFINED)
        if (isinstance(t.get_referent(), StreetReferent)): 
            ...
            if (lin is not None and (lin.length_char < 70)): 
                return FragToken._new1945(t0, lin.end_token, InstrumentKind.CONTACT)
        if (t0.is_char('(')): 
            br = BracketHelper.try_parse(t0, BracketParseAttr.NO, 100)
            if (br is not None and br.is_newline_after): 
                return FragToken._new1945(t0, br.end_token, InstrumentKind.UNDEFINED)
        if (t0.previous is not None and t0.previous.is_table_control_char): 
            rows = TableHelper.try_parse_rows(t0.previous, 0, False, False)
            if (rows is not None and (len(rows) < 30) and len(rows) > 3): 
                ...
                if (tt is not None and tt.is_newline_before): 
                    ...
                    if (dt is not None and dt.typ == DecreeToken.ItemType.TYP): 
                        return FragToken._new1945(t0.previous, tt.previous, InstrumentKind.UNDEFINED)
        return None
    ...
    def _create_editions(t : 'Token') -> 'FragToken':
        ...
        if ((t.is_newline_before and t.is_value("С", None) and t.next0_ is not None) and t.next0_.is_value("ИЗМЕНЕНИЕ", None)): 
            eee = FragToken._new1945(t, t, InstrumentKind.EDITIONS)
            t = t.next0_.next0_
            while t is not None: 
        ...
        if (not t.is_char('(') and not t.is_value("ПРИЛОЖЕНИЕ", "ДОДАТОК")): 
            if (t.is_value("В", "У") and t.next0_ is not None and ((t.next0_.is_value("РЕДАКЦИЯ", "РЕДАКЦІЯ") or t.next0_.is_value("РЕД", None)))): 
                ...
                if (dtt0 is not None): 
                    return FragToken._new1945(t, dtt0.end_token, InstrumentKind.EDITIONS)
        ...
        if (ok and not is_doubt): 
            eds = FragToken._new1945(t0, t, InstrumentKind.EDITIONS)
            eds.referents = list()
            for d in decrs: 
    ...
    def __create_owner(t : 'Token') -> 'FragToken':
        ...
        if (t1 is None): 
            return None
        fr = FragToken._new1969(t, t1, InstrumentKind.ORGANIZATION, True)
        return fr
            ...
            if (isinstance(t.get_referent(), DecreeChangeReferent)): 
                ...
                while True:
                    ...
                    if (t0 is not None and ((t0.end_char + 50) < t1.end_char)): 
                        return MetaToken._new658(t0, t1, dcr)
                    return None
            if (t.end_char > self.end_char): 
                break
            t = t.next0_
    ...
    def _add_child(self, ch : 'FragToken', add_fill_content : bool) -> None:
        if (len(self.children) == 0): 
            if (add_fill_content and (self.begin_char < ch.begin_char)): 
                self.children.append(FragToken._new1945(self.begin_token, ch.begin_token.previous, InstrumentKind.CONTENT))
            self.children.append(ch)
            return
        i = 0
        while i < len(self.children): 
            if (ch.end_char <= self.children[i].begin_char): 
                self.children.insert(i, ch)
                if (add_fill_content): 
                    if (i > 0 and (self.children[i - 1].end_token.next0_.begin_char < ch.begin_token.previous.end_char)): 
                        self.children.insert(i, FragToken._new1945(self.children[i - 1].end_token.next0_, ch.begin_token.previous, InstrumentKind.CONTENT))
                    elif (i == 0 and (self.begin_char < ch.begin_char)): 
                        self.children.insert(0, FragToken._new1945(self.begin_token, ch.begin_token.previous, InstrumentKind.CONTENT))
                return
            i += 1
        self.children.append(ch)
    ...
    def _analize_tables(self) -> bool:
        ...
        if (len(self.children) > 0): 
            ...
            empty = True
            for ch in self.children: 
                if ((ch.kind == InstrumentKind.NOTICE or ch.kind == InstrumentKind.FOOTNOTE or ch.kind == InstrumentKind.EDITIONS) or ch.kind == InstrumentKind.COMMENT): 
                    pass
                else: 
                    empty = False
            if (not empty): 
        ...
        while True:
            ...
            if (tt.is_char(chr(0x1E))): 
                pass
            rows = TableHelper.try_parse_rows(tt, end_char_, False, False)
            if (rows is None or (len(rows) < 2)): 
                itok = InstrToken1.parse(tt, True, None, 0, None, False, 0, False, False)
                if (itok is not None): 
                    if (itok.typ == InstrToken1.Types.TABLE): 
                        head.clear()
                        head.append(itok)
                    elif (len(head) > 0): 
                        if (len(itok.numbers) > 0 or itok.typ != InstrToken1.Types.LINE): 
                            head.clear()
                        else: 
                            head.append(itok)
                    tt = itok.end_token
            ...
            if (not ok): 
                ...
                continue
            tab = FragToken._new1945(rows[0].begin_token, rows[len(rows) - 1].end_token, InstrumentKind.TABLE)
            if (len(head) > 0): 
                tab.begin_token = head[0].begin_token
                NumberingHelper.create_number(tab, head[0])
                if (head[0].num_end_token != head[0].end_token and head[0].num_end_token is not None): 
                    fr_name = FragToken._new1952(head[0].num_end_token.next0_, head[0].end_token, InstrumentKind.NAME, True, head[0])
                    tab.children.append(fr_name)
                    tab.name = FragToken._get_restored_namemt(fr_name, False)
                    ii = ContentAnalyzeWhapper.correct_name(tab, fr_name, head, 0)
            self._add_child(tab, True)
            i = 0
            while i < len(rows): 
                rr = FragToken._new1967(rows[i].begin_token, rows[i].end_token, InstrumentKind.TABLEROW, i + 1)
                ...
                for ce in rows[i].cells: 
                    no += 1
                    cell = FragToken._new1967(ce.begin_token, ce.end_token, InstrumentKind.TABLECELL, no)
                    if (ce.col_span > 1): 
                ...
                if (tab.table_columns < cols): 
                    tab.table_columns = cols
                tt = rows[i].end_token
                i += 1
            if (tab.table_columns > 1): 
                rnums = Utils.newArray(tab.table_columns, 0)
                rnums_cols = Utils.newArray(tab.table_columns, 0)
        ...
        if ((t0 is not None and (t0.end_char < self.end_char) and tabs) and t0 != self.end_token): 
            if (len(self.children) > 0 and (t0.end_char < self.children[len(self.children) - 1].end_char)): 
                pass
            else: 
                self._add_child(FragToken._new1945(t0, self.end_token, InstrumentKind.CONTENT), False)
        return tabs
        ...
        while t is not None and (cou < 30): 
            ...
            if (t.is_value("ОТВЕТ", None)): 
                if (t.is_newline_after): 
                    ...
                    ok = True
            if (ok): 
                res = FragToken._new1945(t0, li.end_token, InstrumentKind.HEAD)
                if (li.begin_token != t0): 
                    hh = FragToken._new1945(t0, li.begin_token.previous, InstrumentKind.APPROVED)
                    res.children.append(hh)
                res.children.append(FragToken._new1972(li.begin_token, li.end_token, InstrumentKind.KEYWORD, True))
                return res
            t = li.end_token
            t = t.next0_; cou += 1
        return None
    ...
    def __create_contract_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (t1 is None): 
            return None
        p1 = InstrumentParticipantReferent._new2053(par1.typ)
        ...
        if (par1.kind == ParticipantToken.Kinds.NAMEDASPARTS): 
            ...
            while ii < len(par1.parts): 
                pp = InstrumentParticipantReferent._new2053("СТОРОНА {0}".format(ii + 1))
        ...
        title = FragToken._new1945(t0, t0, InstrumentKind.HEAD)
        add = False
        nam_beg = None
        nam_end = None
        dttyp = None
            ...
            if ((dttyp is not None and nam_beg is None and t.chars.is_cyrillic_letter) and (isinstance(t, TextToken))): 
                if (t.is_value("МЕЖДУ", "МІЖ")): 
                    ...
                    if (pp is not None and pp.end_token.next0_ is not None and pp.end_token.next0_.is_and): 
                        ...
                        if (pp2 is not None): 
                            fr = FragToken._new1945(t, pp2.end_token, InstrumentKind.PLACE)
        ...
        if (nam_end is not None and nam_beg is not None): 
            ...
            if (val is not None and len(val) > 3): 
                nam = FragToken._new2007(nam_beg, nam_end, InstrumentKind.NAME, val)
                ...
                if (nam_beg2 is not None and nam_end2 is not None): 
                    ...
                    if (val2 is not None and len(val2) > 3): 
                        nam = FragToken._new2007(nam_beg2, nam_end2, InstrumentKind.NAME, val2)
                        title.children.append(nam)
                        title.sort_children()
        ...
        if (len(title.children) > 0 and title.children[0].begin_char > title.begin_char): 
            title.children.insert(0, FragToken._new1945(title.begin_token, title.children[0].begin_token.previous, InstrumentKind.UNDEFINED))
    ...
    def __create_project_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        while True:
            ...
            if ((isinstance(t, TextToken)) and ((t.term == "ПРОЕКТ" or t.term == "ЗАКОНОПРОЕКТ"))): 
                if ((t.is_value("ПРОЕКТ", None) and t == t0 and (isinstance(t.next0_, ReferentToken))) and (isinstance(t.next0_.get_referent(), OrganizationReferent))): 
                    return None
                is_project = True
                title.children.append(FragToken._new1969(t, t, InstrumentKind.KEYWORD, True))
                doc.add_slot(InstrumentReferent.ATTR_TYPE, "ПРОЕКТ", False, 0)
                continue
            tt = FragToken.__attach_project_enter(t)
            if (tt is not None): 
                is_entered = True
                title.children.append(FragToken._new1945(t, tt, InstrumentKind.APPROVED))
                t = tt
                continue
            tt = FragToken.__attach_project_misc(t)
            if (tt is not None): 
                title.children.append(FragToken._new1945(t, tt, (InstrumentKind.EDITIONS if tt.is_value("ЧТЕНИЕ", "ЧИТАННЯ") else InstrumentKind.UNDEFINED)))
                t = tt
                continue
        ...
        if (t11 is None): 
            return None
        nam = FragToken._new1969(t00, t11, InstrumentKind.NAME, True)
    ...
    def __create_justice_participants(title : 'FragToken', doc : 'InstrumentReferent') -> None:
        ...
        while t is not None:
            ...
            if (tmp.tell() > 10 and t1 is not None): 
                art = InstrumentArtefactReferent._new2068("предмет")
    ...
    def __create_just_participant(t : 'Token', typ : str) -> 'ReferentToken':
        ...
        if (not ok): 
            return None
        pat = InstrumentParticipantReferent._new2053(typ)
        for r in refs: 
            ...
            while True:
                ...
                if (t.is_value("ВЗЫСКАТЬ", "СТЯГНУТИ")): 
                    ...
                    if (sum0_ is not None): 
                        art = InstrumentArtefactReferent._new2068("РЕЗОЛЮЦИЯ")
                        if (gosposh): 
            ...
            if (self.children[i].kind == InstrumentKind.DIRECTIVE and ((i + 1) < len(self.children))): 
                ...
                if ((((s == "РЕШЕНИЕ" or s == "ОПРЕДЕЛЕНИЕ" or s == "ПОСТАНОВЛЕНИЕ") or s == "ПРИГОВОР" or s == "РІШЕННЯ") or s == "ВИЗНАЧЕННЯ" or s == "ПОСТАНОВА") or s == "ВИРОК"): 
                    ...
                    if (ii == (i + 1)): 
                        ...
                        res.append(FragToken._new1945(self.children[i + 1].begin_token, self.children[ii].end_token, InstrumentKind.CONTENT))
                else: 
                    dir0_ = True
        if (res is not None): 
            return res
    ...
    def __create_action_question(t : 'Token', max_char : int) -> 'FragToken':
        ...
        if (res is None): 
            return None
        ques = FragToken._new1945(t, t2.previous, InstrumentKind.QUESTION)
        res.children.insert(0, ques)
        ques.children.append(FragToken._new1945(li[len(li) - 1].begin_token, li[len(li) - 1].end_token, InstrumentKind.KEYWORD))
        content = FragToken._new1945(li[len(li) - 1].end_token.next0_, t2.previous, InstrumentKind.CONTENT)
        ques.children.append(content)
        content._analize_content(res, max_char > 0, InstrumentKind.UNDEFINED)
        if (len(li) > 1): 
            fr = FragToken._new1972(t, li[len(li) - 2].end_token, InstrumentKind.NAME, True)
    ...
    def __create_gost_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        while t is not None: 
            ...
            if (t.is_newline_before and t.is_value("ПРЕДИСЛОВИЕ", None) and t.is_newline_after): 
                empty = FragToken._new1945(t, t, InstrumentKind.IGNORED)
    ...
    def __create_doc_title_(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (t is not None and t.is_value("ДЕЛО", "СПРАВА")): 
            ...
            if (dt is not None and dt.typ == DecreeToken.ItemType.NUMBER): 
                dt.begin_token = t
                title.children.append(FragToken._new2007(t, t, InstrumentKind.KEYWORD, "ДЕЛО"))
                FragToken.__add_title_attr(doc, title, dt)
                t = dt.end_token.next0_
        ...
        while True:
            ...
            if (t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))): 
                ...
                if (iii is not None and iii.typ == InstrToken1.Types.COMMENT): 
                    cmt = FragToken._new1945(iii.begin_token, iii.end_token, InstrumentKind.COMMENT)
                ...
                if (iii is not None and iii.end_token.is_char('?')): 
                    cmt = FragToken._new1945(iii.begin_token, iii.end_token, InstrumentKind.NAME)
                ...
                if ((((t.is_value("ЗАЯВИТЕЛЬ", "ЗАЯВНИК") or t.is_value("ИСТЕЦ", "ПОЗИВАЧ") or t.is_value("ОТВЕТЧИК", "ВІДПОВІДАЧ")) or t.is_value("ДОЛЖНИК", "БОРЖНИК") or t.is_value("КОПИЯ", "КОПІЯ"))) and t.next0_ is not None and ((t.next0_.is_char(':') or t.next0_.is_table_control_char))): 
                    ...
                    if (ptt is not None): 
                        ...
                        if (t1.next0_ is not None and t1.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                t1 = br.end_token
                        ft = FragToken._new1945(t, t1, InstrumentKind.INITIATOR)
                ...
                if (t.is_value("ЦЕНА", "ЦІНА") and t.next0_ is not None and t.next0_.is_value("ИСК", "ПОЗОВ")): 
                    ...
                    if (tt is not None and has_money): 
                        ...
                        if (tt.next0_ is not None and tt.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                tt = br.end_token
                        title.children.append(FragToken._new1945(t, tt, InstrumentKind.CASEINFO))
                        t1 = tt
                        title.end_token = t1
                        t = title.end_token
                        continue
                if (t.is_value("В", "У")): 
                    ...
                    if (tt is not None and (isinstance(tt.get_referent(), OrganizationReferent))): 
                        ...
                        if (t1.next0_ is not None and t1.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                t1 = br.end_token
                        ooo = FragToken._new1945(t, t1, InstrumentKind.ORGANIZATION)
                        ooo.referents = list()
                ...
                if (t.length_char == 1 and t.chars.is_letter and t.is_whitespace_after): 
                    ...
                    while ii < len(InstrToken._m_directives_norm): 
                        ...
                        if (ee is not None and ee.is_newline_after): 
                            ooo = FragToken._new2007(t, ee, InstrumentKind.KEYWORD, InstrToken._m_directives_norm[ii])
                            title.children.append(ooo)
            ...
            if (dr0 is not None): 
                if (doc.typ is None or doc.typ == dr0.typ): 
                    ...
                    if (li is not None and len(li) > 0 and li[len(li) - 1].is_newline_after): 
                        ...
                        if (doc.typ == "КОДЕКС"): 
                            ...
                            if (pt is not None and len(pt.values) > 0): 
                                ...
                                title.children.append(FragToken._new2007(pt.begin_token, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
                                t = pt.end_token
                                continue
                    ...
                    t1 = t
                    title.children.append(FragToken._new2007(t, t, InstrumentKind.NAME, nam))
                    if (pt is not None and len(pt.values) > 0): 
                        title.children.append(FragToken._new2007(pt.begin_token, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
                        t1 = pt.end_token
                    t = t1
            ...
            if (doc.typ is None): 
                ...
                if (ttt1 is not None and ttt1.is_newline_after): 
                    ...
                    if (doc.typ is None): 
                        doc.typ = typ
                    title.children.append(FragToken._new2007(t, ttt1, InstrumentKind.TYP, typ))
                    dt0 = DecreeToken._new1097(t, ttt1, DecreeToken.ItemType.TYP, typ)
                    can_be_orgs = False
                    t = ttt1
                    t1 = t
            ...
            if (dt is None and dt0 is not None and ((dt0.typ == DecreeToken.ItemType.OWNER or dt0.typ == DecreeToken.ItemType.ORG))): 
                if ((isinstance(t, NumberToken)) and t.is_newline_after and t.is_newline_before): 
                    dt = DecreeToken._new1097(t, t, DecreeToken.ItemType.NUMBER, str(t.value))
            ...
            if (dt is None): 
                ...
                if (t.is_value("ДАТА", None)): 
                    ...
                    while True:
                        ...
                        if (da is not None): 
                            frdt = FragToken._new1945(t, tt, InstrumentKind.DATE)
                ...
                if (((isinstance(r, AddressReferent)) or (isinstance(r, UriReferent)) or (isinstance(r, PhoneReferent))) or (isinstance(r, PersonIdentityReferent)) or (isinstance(r, BankDataReferent))): 
                    cnt = FragToken._new1945(t, t, InstrumentKind.CONTACT)
                    cnt.referents = list()
            ...
            if (t == t0): 
                ...
                if (nam is not None): 
                    ...
                    if (not Utils.isNullOrEmpty(name_)): 
                        ...
                        title.children.append(FragToken._new2007(t0, t1, InstrumentKind.NAME, name_.strip()))
            ...
            if (t is not None and t.is_value("О", None)): 
                ...
                if (nam is not None): 
                    ...
                    if (not Utils.isNullOrEmpty(name_)): 
                        t1 = nam.end_token
                        doc.add_slot(InstrumentBlockReferent.ATTR_NAME, name_.strip(), True, 0)
                        title.children.append(FragToken._new2007(t, t1, InstrumentKind.NAME, name_.strip()))
            if (attrs > 0): 
                title.end_token = t1
                return title
            return None
        ...
        if (end_empty_lines is not None and doc.find_slot(InstrumentReferent.ATTR_SOURCE, None, True) is None): 
            ...
            title.children.insert(0, FragToken._new2007(t0, end_empty_lines, InstrumentKind.ORGANIZATION, val))
        is_case = False
        for ch in title.children: 
        ...
        if ((((name_ is not None or t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))) or ((not t.is_newline_before and len(title.children) > 0 and title.children[len(title.children) - 1].kind == InstrumentKind.TYP)))) and not is_case): 
            ...
            if (val is not None and tt0.begin_char <= t1.begin_char): 
                ...
                title.children.append(FragToken._new2007(tt0, t1, InstrumentKind.NAME, val.strip()))
                if ("КОДЕКС" in val): 
                    npt = NounPhraseHelper.try_parse(tt0, NounPhraseParseAttr.NO, 0, None)
        ...
        while True:
            ...
            if (t1.is_newline_before and (isinstance(t1.get_referent(), DecreeReferent)) and t1.is_newline_after): 
                dr = Utils.asObjectOrNull(t1.get_referent(), DecreeReferent)
                title.children.append(FragToken._new1945(t1, t1, InstrumentKind.IGNORED))
                continue
            if (t1.is_newline_before and t1.is_value("ЧАСТЬ", "ЧАСТИНА")): 
                ...
                if (pt is not None and pt.is_newline_after): 
                    ...
                    if (pt2 is not None and (((pt2.typ == PartToken.ItemType.SECTION or pt2.typ == PartToken.ItemType.SUBSECTION or pt2.typ == PartToken.ItemType.CHAPTER) or pt2.typ == PartToken.ItemType.CLAUSE))): 
                        ...
                        doc.add_slot(InstrumentReferent.ATTR_PART, pt.values[0].value, False, 0)
                        title.children.append(FragToken._new2007(t1, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
            ...
            if (t1.is_newline_before): 
                ...
                if (iii is not None and iii.typ == InstrToken1.Types.COMMENT): 
                    title.children.append(FragToken._new1945(t1, iii.end_token, InstrumentKind.COMMENT))
                    t1 = iii.end_token
                    continue
            ...
            if ((isinstance(t1.get_referent(), DecreeReferent)) and t1.get_referent().kind == DecreeKind.PUBLISHER and t1.is_newline_after): 
                pub = FragToken._new1945(t1, t1, InstrumentKind.APPROVED)
                pub.referents = list()
            ...
            if (tt.is_char_of("([") and tt.is_newline_before): 
                ...
                if (br is not None): 
                    ...
                    t1 = title.end_token
                    title.children.append(FragToken._new1945(br.begin_token, br.end_token, (InstrumentKind.NAME if tt.is_char('[') else InstrumentKind.COMMENT)))
                    continue
            if (tt.is_table_control_char): 
                title.end_token = tt
                continue
    ...
    def __create_appendix_title(t0 : 'Token', app : 'FragToken', doc : 'InstrumentReferent', is_app : bool, start : bool) -> 'FragToken':
        ...
        if (rr is not None): 
            if (rr.type_name == "PERSON"): 
                return None
        title = FragToken._new1945(t0, t, InstrumentKind.HEAD)
        ...
        while True:
            ...
            if (fr.typ == InstrToken1.Types.COMMENT): 
                t = fr.end_token
                title.end_token = t
                title.children.append(FragToken._new1945(fr.begin_token, fr.end_token, InstrumentKind.COMMENT))
                continue
            ...
            if (t.is_value("ОСОБЫЙ", "ОСОБЛИВИЙ") and t.next0_ is not None): 
                t2 = t.next0_
            if (isinstance(t, TextToken)): 
                title.children.append(FragToken._new1969(t, t2, InstrumentKind.KEYWORD, True))
            t = fr.end_token
            title.end_token = t
            if (fr.typ == InstrToken1.Types.APPENDIX and fr.num_begin_token is None): 
                ...
                if (fr1 is not None and fr1.typ == InstrToken1.Types.APPROVED): 
                    t = fr1.begin_token
                    title.children.append(FragToken._new2007(t, t, InstrumentKind.KEYWORD, t.get_source_text().upper()))
                    t = fr1.end_token
                    title.end_token = t
                    fr = fr1
            appr0 = FragToken.__create_approved(t)
            ...
            if (fr.num_begin_token is not None and fr.num_end_token is not None): 
                num = FragToken._new2007(fr.num_begin_token, fr.num_end_token, InstrumentKind.NUMBER, MiscHelper.get_text_value(fr.num_begin_token, fr.num_end_token, GetTextAttr.KEEPREGISTER))
                ...
                if (t.get_referent().kind == DecreeKind.PUBLISHER): 
                    ff = FragToken._new1945(t, t, InstrumentKind.APPROVED)
                    ff.referents = list()
                    ...
                    appr = FragToken._new1945(kw.begin_token, t, InstrumentKind.APPROVED)
                    del title.children[len(title.children) - 1]
                    appr.children.append(kw)
                    appr.children.append(FragToken._new1945(t, t, InstrumentKind.DOCREFERENCE))
                    title.children.append(appr)
                else: 
                    title.children.append(FragToken._new1945(t, t, InstrumentKind.DOCREFERENCE))
            elif (fr.typ == InstrToken1.Types.APPROVED and fr.length_char > 15 and fr.begin_token != fr.end_token): 
                title.children.append(FragToken._new1945(fr.begin_token.next0_, t, InstrumentKind.DOCREFERENCE))
            else: 
                ...
                if (dts is not None and len(dts) > 0 and dts[0].typ == DecreeToken.ItemType.TYP): 
                    dref = FragToken._new1945(dts[0].begin_token, dts[0].end_token, InstrumentKind.DOCREFERENCE)
                    i = 1
                    while i < len(dts): 
        ...
        if (t.is_value("К", "ДО")): 
            ...
            if (max_ind >= 0): 
                ...
                te = t.next0_
            if (te is not None): 
                dr = FragToken._new1945(t, te, InstrumentKind.DOCREFERENCE)
        ...
        while True:
            ...
            if (t.is_newline_before or t.previous.is_table_control_char): 
                ...
                if (ltt.typ == InstrToken1.Types.APPROVED): 
                    title.children.append(FragToken._new1945(ltt.begin_token, ltt.begin_token, InstrumentKind.APPROVED))
                    if (ltt.begin_token != ltt.end_token): 
                        title.children.append(FragToken._new1945(ltt.begin_token.next0_, ltt.end_token, InstrumentKind.DOCREFERENCE))
                    t = ltt.end_token
        ...
        while title.end_token.next0_ is not None:
            ...
            if (misc is not None): 
                ...
                continue
            ltt = InstrToken1.parse(title.end_token.next0_, True, None, 0, None, False, 0, False, False)
            if (ltt is None): 
                break
            if (ltt.is_pure_hiphen_line): 
                title.end_token = ltt.end_token
                t = title.end_token
                continue
            if (ltt.typ == InstrToken1.Types.FOOTNOTE): 
                fnn = FragToken._new1945(ltt.begin_token, ltt.end_token, InstrumentKind.FOOTNOTE)
                NumberingHelper.create_number(fnn, ltt)
                title.children.append(fnn)
                title.end_token = ltt.end_token
                t = title.end_token
                continue
            break
    ...
    def __add_title_attr(doc : 'InstrumentReferent', title : 'FragToken', dt : 'DecreeToken') -> bool:
        if (dt.typ == DecreeToken.ItemType.TYP): 
            ...
            if (title is not None): 
                title.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.TYP, Utils.ifNotNull(dt.full_value, dt.value)))
            ...
            if (dt.is_delo): 
                if (doc is not None): 
                    ...
                    if (doc.reg_number == dt.value): 
                        doc.reg_number = None
                if (title is not None): 
                    title.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.CASENUMBER, dt.value))
            else: 
                ...
                if (title is not None): 
                    title.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.NUMBER, dt.value))
            ...
            if (title is not None): 
                title.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.NAME, dt.value))
        elif (dt.typ == DecreeToken.ItemType.DATE): 
            if (doc is None or doc._add_date(dt)): 
                if (title is not None): 
                    title.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.DATE, dt))
        elif (dt.typ == DecreeToken.ItemType.TERR): 
            if (title is not None): 
                title.children.append(FragToken._new2007(dt.begin_token, dt.end_token, InstrumentKind.PLACE, dt))
            ...
            if (title is not None): 
                title.children.append(FragToken._new2007(dt.begin_token, dt.end_token, (InstrumentKind.ORGANIZATION if dt.typ == DecreeToken.ItemType.ORG else InstrumentKind.INITIATOR), dt))
            if (doc is not None): 
    ...
    def __createtztitle(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (tz is None): 
            return None
        title = FragToken._new1945(t0, tz.end_token, InstrumentKind.HEAD)
        t = t0
        first_pass4118 = True
        while True:
            ...
            if (dt is not None): 
                ...
                if (br is not None and BracketHelper.is_bracket(t.next0_, True)): 
                    nam = FragToken._new1972(br.begin_token, br.end_token, InstrumentKind.NAME, True)
                    title.children.append(nam)
                    t = br.end_token
                    title.end_token = t
                    continue
                if (t.next0_ is not None and t.next0_.is_value("НА", None)): 
                    ...
                    while True:
                        ...
                        if (npt is not None): 
                            tt = npt.end_token
                        t1 = tt
                    nam = FragToken._new1972(t.next0_, t1, InstrumentKind.NAME, True)
                    title.children.append(nam)
                    t = t1
                    title.end_token = t
                    continue
    ...
    @staticmethod
    def _new1945(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        return res
    
    @staticmethod
    def _new1951(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrToken1', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._itok = _arg3
    ...
    @staticmethod
    def _new1952(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val2 = _arg4
        res._itok = _arg5
    ...
    @staticmethod
    def _new1959(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._itok = _arg3
        return res
    
    @staticmethod
    def _new1960(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._itok = _arg4
        return res
    ...
    @staticmethod
    def _new1967(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : int) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.number = _arg4
        return res
    ...
    @staticmethod
    def _new1969(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val2 = _arg4
        return res
    ...
    @staticmethod
    def _new1972(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val = _arg4
        return res
    ...
    @staticmethod
    def _new1984(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : object, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.value = _arg4
        res._itok = _arg5
    ...
    @staticmethod
    def _new1995(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : int, _arg5 : bool, _arg6 : typing.List['Referent']) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.number = _arg4
    ...
    @staticmethod
    def _new2007(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : object) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new2013(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentReferent', _arg4 : 'InstrumentKind') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._m_doc = _arg3
        res.kind = _arg4
        return res
    ...
    @staticmethod
    def _new2190(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val = _arg4
        res._itok = _arg5
