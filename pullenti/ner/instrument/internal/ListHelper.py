...
class ListHelper:
                        ...
                        if ((ord(list_bullet)) != 0 and ch.begin_token.is_char(list_bullet)): 
                            ...
                            while tt is not None and (tt.end_char < ch.end_char): 
                                if (tt.previous.is_char('.') and MiscHelper.can_be_start_of_sentence(tt)): 
                                    ch2 = FragToken._new1967(tt, ch.end_token, InstrumentKind.INDENTION, ch.number)
    ...
    def __analize_list_items(chi : typing.List['FragToken'], ind : int) -> int:
        ...
        if (res.has_changes and res.multiline_changes_value is not None): 
            ...
            cit = FragToken._new1945(ci.begin_token, ci.end_token, InstrumentKind.CITATION)
        ...
        for li in lines: 
            ...
            ch = FragToken._new1967(li.begin_token, li.end_token, (InstrumentKind.LISTITEM if li.is_list_item else InstrumentKind.CONTENT), li.number)
            ...
            if (chli is not None): 
                for lt in chli: 
                    ch.children.append(FragToken._new1945(lt.begin_token, lt.end_token, InstrumentKind.LISTITEM))
                if (ch.begin_char < ch.children[0].begin_char): 
                    ch.children.insert(0, FragToken._new1945(ch.begin_token, ch.children[0].begin_token.previous, InstrumentKind.CONTENT))
        return ret
            ...
            if ((lines[i].typ == InstrToken1.Types.LINE and len(lines[i].numbers) == 0 and lines[i].begin_token.is_value("ПРИЛОЖЕНИЯ", "ДОДАТОК")) and len(lines[i + 1].numbers) > 0 and lines[i].end_token.is_char(':')): 
                ...
                while True:
                    ...
                    if (len(lines[i].numbers) == 0): 
                        ...
                        nn = 0
                        wrapnn2181 = RefOutArgWrapper(0)
                        inoutres2182 = Utils.tryParseInt(lines[i].numbers[0], wrapnn2181)
                        nn = wrapnn2181.value
                        if (inoutres2182): 
                            num = (nn + 1)
    ...
    def correct_index(lines : typing.List['InstrToken1']) -> None:
        ...
        if (len(index) == len(content) and len(index) > 2): 
            if ((ind_text * 10) < con_text): 
                lines[0] = InstrToken1._new2183(lines[0].begin_token, lines[cind - 1].end_token, True, InstrToken1.Types.INDEX)
