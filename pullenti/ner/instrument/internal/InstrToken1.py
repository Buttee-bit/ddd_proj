...
class InstrToken1(MetaToken):
    ...
    @property
    def is_pure_hiphen_line(self) -> bool:
        tt = self.begin_token
        while tt is not None and tt.end_char <= self.end_char: 
            if (tt.is_hiphen or tt.is_char('_')): 
                pass
            else: 
                return False
            tt = tt.next0_
    ...
    def is_italic_font(self) -> bool:
        st = self.begin_token.kit.sofa.styles
        if (st is None): 
            return False
        yes = 0
        no = 0
    ...
    def __str__(self) -> str:
        ...
        if (self.is_italic_font): 
            print(" Italic", end="", file=res)
        if (self.is_bold_font): 
            print(" Bold", end="", file=res)
    ...
    def parse(t : 'Token', ignore_directives : bool, cur : 'FragToken'=None, lev : int=0, prev : 'InstrToken1'=None, is_citat : bool=False, max_char : int=0, can_be_table_cell : bool=False, is_in_index : bool=False) -> 'InstrToken1':
        ...
        if (t.is_char_of("({")): 
            ...
            if (fr is not None): 
                edt = InstrToken1._new1944(fr.begin_token, fr.end_token, InstrToken1.Types.EDITIONS)
            else: 
                t2 = InstrToken1._create_edition(t)
                if (t2 is not None): 
                    edt = InstrToken1._new1944(t, t2, InstrToken1.Types.EDITIONS)
            if (edt is not None): 
                if (edt.end_token.next0_ is not None and edt.end_token.next0_.is_char('.')): 
                    edt.end_token = edt.end_token.next0_
                tt23 = edt.begin_token
                while tt23 is not None and (tt23.end_char < edt.end_char): 
                    if (tt23.is_value2("УТРАТИТЬ", "СИЛУ") or tt23.is_value("ИСКЛЮЧИТЬ", None)): 
                        edt.is_expired = True
                    tt23 = tt23.next0_
                return edt
        t0 = t
        t00 = None
        res = InstrToken1._new2166(t0, t, True)
        while t is not None: 
        ...
        if (dt is None and (((isinstance(t.get_referent(), PersonReferent)) or (isinstance(t.get_referent(), InstrumentParticipantReferent))))): 
            dt = DecreeToken._new1098(t, t, DecreeToken.ItemType.OWNER)
        ...
        if (t.is_value("КОНСУЛЬТАНТПЛЮС", None) or t.is_value("ГАРАНТ", None) or ((t.is_value("ИНФОРМАЦИЯ", None) and t.is_newline_before))): 
            ...
            if (t1 is not None and ((t1.is_value("ПРИМЕЧАНИЕ", None) or ok))): 
                if (t1.next0_ is not None and t1.next0_.is_char('.')): 
                    t1 = t1.next0_
                re = InstrToken1._new1944(t, t1, InstrToken1.Types.COMMENT)
                hiph = False
                t1 = t1.next0_
                first_pass4131 = True
                while True:
        ...
        if (check_comment > 0 or t.is_value("О", "ПРО")): 
            ...
            while ttt is not None: 
                ...
                ttt = ttt.next0_
            if (ok): 
                cmt = InstrToken1._new1944(t, t1, InstrToken1.Types.COMMENT)
        ...
        if (tt is not None): 
            ...
            if (dt1 is not None and dt1.typ == DecreeToken.ItemType.TYP): 
                ...
                return res
        val = None
        wrapval2175 = RefOutArgWrapper(None)
        tt2 = InstrToken1._check_directive(t, wrapval2175)
        val = wrapval2175.value
        if (tt2 is not None): 
            if (tt2.is_newline_after or ((tt2.next0_ is not None and ((tt2.next0_.is_char_of(":") or ((tt2.next0_.is_char('.') and tt2 != t)))) and ((tt2.next0_.is_newline_after or t.chars.is_all_upper))))): 
                return InstrToken1._new2170(t, (tt2 if tt2.is_newline_after else tt2.next0_), InstrToken1.Types.DIRECTIVE, val)
            ...
            if (tt is not None and tt.is_char_of(".)]")): 
                ...
                while tt is not None: 
                    ...
                    if (tt.is_newline_after): 
                        if (ok): 
                            return InstrToken1._new1944(t, tt, InstrToken1.Types.COMMENT)
                        break
                    tt = tt.next0_
        multi_nums = False
        if (t is not None and (((t.is_newline_before or is_in_index or is_citat) or ((t.previous is not None and t.previous.is_table_control_char)))) and not t.is_table_control_char): 
            ...
            if (ok and (isinstance(t, TextToken))): 
                ...
                if (s == "ГЛАВА" or s == "ГОЛОВА"): 
                    ...
                    ok = True
                    multi_nums = True
                elif (s == "ТАБЛИЦА" or s == "НОРМА"): 
                    res.typ = InstrToken1.Types.TABLE
                    t = t.next0_
                    ok = True
                if (ok): 
                    ttt = t
        ...
        while True:
            ...
            if (t.is_char('(')): 
                ...
                if (br is not None): 
                    ...
                    continue
            if (num): 
                if (res.typ == InstrToken1.Types.TABLE): 
            ...
            if (t != res.begin_token and InstrToken1.__is_first_line(t)): 
                break
            tmp = None
            wraptmp2172 = RefOutArgWrapper(None)
            tt2 = InstrToken1._check_directive(t, wraptmp2172)
            tmp = wraptmp2172.value
            if (tt2 is not None): 
        ...
        if (res.typ == InstrToken1.Types.LINE): 
            if (res.num_typ != NumberTypes.UNDEFINED): 
                ...
                for nn in res.numbers: 
                    vv = 0
                    wrapvv2173 = RefOutArgWrapper(0)
                    inoutres2174 = Utils.tryParseInt(nn, wrapvv2173)
                    vv = wrapvv2173.value
                    if (inoutres2174): 
    ...
    def _create_edition(t : 'Token') -> 'Token':
        ...
        t1 = t
        br = 0
        if (t.is_char('(') and ((t.is_newline_before or ((t.previous is not None and t.previous.is_char(')')))))): 
            ok = True
            br = 1
            t1 = t.next0_
    ...
    @staticmethod
    def _new1944(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Types') -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new2166(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.all_upper = _arg3
        return res
    
    @staticmethod
    def _new2170(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Types', _arg4 : str) -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.typ = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new2183(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : 'Types') -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
