...
class NumberingHelper:
        ...
        if ((len(prev.numbers) - 1) == len(next0_.numbers) and len(prev.numbers) > 1): 
            ...
            while i < (len(prev.numbers) - 2): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12184 = RefOutArgWrapper(0)
            inoutres2185 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 2], wrapn12184)
            n1 = wrapn12184.value
            if (not inoutres2185): 
        ...
        if ((len(prev.numbers) - 2) == len(next0_.numbers) and len(prev.numbers) > 2): 
            ...
            while i < (len(prev.numbers) - 3): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12186 = RefOutArgWrapper(0)
            inoutres2187 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 3], wrapn12186)
            n1 = wrapn12186.value
            if (not inoutres2187): 
                return 0
            if ((n1 + 1) != n2): 
        ...
        if ((len(prev.numbers) - 3) == len(next0_.numbers) and len(prev.numbers) > 3): 
            ...
            while i < (len(prev.numbers) - 4): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12188 = RefOutArgWrapper(0)
            inoutres2189 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 4], wrapn12188)
            n1 = wrapn12188.value
            if (not inoutres2189): 
                return 0
            if ((n1 + 1) != n2): 
    ...
    def create_number(owner : 'FragToken', itok : 'InstrToken1') -> None:
        if ((itok.num_begin_token is None or itok.num_end_token is None or itok.numbers is None) or len(itok.numbers) == 0): 
            return
        num = FragToken._new2190(itok.num_begin_token, itok.num_end_token, InstrumentKind.NUMBER, True, itok)
    ...
    def parse_number(t : 'Token', res : 'InstrToken1', prev : 'InstrToken1') -> None:
        ...
        if ((len(res.numbers) == 0 and res.typ == InstrToken1.Types.LINE and res.begin_token.is_char('<')) and res.begin_token.next0_ is not None): 
            tt = res.begin_token.next0_
            if ((isinstance(tt, NumberToken)) and tt.next0_ is not None and tt.next0_.is_char('>')): 
                res.typ = InstrToken1.Types.FOOTNOTE
                res.numbers.append(tt.value)
                res.num_begin_token = tt.previous
                res.num_end_token = tt.next0_
                ...
                while tt2 is not None: 
                    if (tt2.is_char('*')): 
                        if (tt2.next0_ is not None and tt2.next0_.is_char('>')): 
                            res.typ = InstrToken1.Types.FOOTNOTE
                            res.numbers.append(str(num))
                            res.num_begin_token = tt.previous
                            res.num_end_token = tt2.next0_
                            res.end_token = tt2.next0_
                    else: 
                        break
                    tt2 = tt2.next0_; num += 1
    ...
    def create_diap(s1 : str, s2 : str) -> typing.List[str]:
        ...
        if (s2.startswith(s1)): 
            ...
            if (((i + 1) < len(s2)) and s2[i] == '.' and str.isdigit(s2[i + 1])): 
                wrapn22191 = RefOutArgWrapper(0)
                inoutres2192 = Utils.tryParseInt(s2[i + 1:], wrapn22191)
                n2 = wrapn22191.value
                if (inoutres2192): 
                    res0 = list()
                    res0.append(s1)
        ...
        if (((i)) > 0): 
            pref = s1[0:0+i + 1]
            wrapn12195 = RefOutArgWrapper(0)
            inoutres2196 = Utils.tryParseInt(s1[i + 1:], wrapn12195)
            n1 = wrapn12195.value
            if (not inoutres2196): 
                return None
            if (not s2.startswith(pref)): 
                return None
            wrapn22193 = RefOutArgWrapper(0)
            inoutres2194 = Utils.tryParseInt(s2[i + 1:], wrapn22193)
            n2 = wrapn22193.value
            if (not inoutres2194): 
                return None
        else: 
            wrapn12199 = RefOutArgWrapper(0)
            inoutres2200 = Utils.tryParseInt(s1, wrapn12199)
            n1 = wrapn12199.value
            if (not inoutres2200): 
                return None
            wrapn22197 = RefOutArgWrapper(0)
            inoutres2198 = Utils.tryParseInt(s2, wrapn22197)
            n2 = wrapn22197.value
            if (not inoutres2198): 
                return None
        if (n2 <= n1): 
