...
class TitlePageAnalyzer(Analyzer):
    ...
    def process_referent1(self, begin : 'Token', end : 'Token') -> 'ReferentToken':
        et = None
        wrapet3443 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(begin, (0 if end is None else end.end_char), begin.kit, wrapet3443)
        et = wrapet3443.value
        if (tpr is None): 
            return None
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        et = None
        wrapet3444 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(kit.first_token, 0, kit, wrapet3444)
        et = wrapet3444.value
        if (tpr is not None): 
            ...
            for r in rli: 
                ...
                if (isinstance(r, OrganizationReferent)): 
                    ...
                    if ("курс" in org0_.types and org0_.number is not None): 
                        i = 0
                        wrapi3445 = RefOutArgWrapper(0)
                        inoutres3446 = Utils.tryParseInt(org0_.number, wrapi3445)
                        i = wrapi3445.value
                        if (inoutres3446): 
                            if (i > 0 and (i < 8)): 
                                res.student_year = i
