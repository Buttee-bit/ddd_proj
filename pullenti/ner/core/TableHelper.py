...
class TableHelper:
    ...
    def try_parse_rows(t : 'Token', max_char : int, must_be_start_of_table : bool, always : bool=False) -> typing.List['TableRowToken']:
        ...
        if (must_be_start_of_table): 
            if (not t.is_char(chr(0x1E))): 
                return None
            is_tab = True
        wrapis_tab733 = RefOutArgWrapper(is_tab)
        rw = TableHelper.__parse(t, max_char, None, wrapis_tab733)
        is_tab = wrapis_tab733.value
        if (rw is None): 
            return None
        res = list()
        res.append(rw)
        ...
        while True:
            ...
            if (t.is_char(chr(0x1E))): 
                ...
                if (rows0 is not None and rows0[len(rows0) - 1].end_token.is_char(chr(0x1F))): 
                    ...
                    continue
            wrapis_tab732 = RefOutArgWrapper(is_tab)
            rw0 = TableHelper.__parse(t, max_char, rw, wrapis_tab732)
            is_tab = wrapis_tab732.value
            if (rw0 is None): 
    ...
    def __parse(t : 'Token', max_char : int, prev : 'TableRowToken', is_tab : bool) -> 'TableRowToken':
        ...
        res.cells.append(TableCellToken._new734(t, tt, cell_info.row_span, cell_info.col_span))
        tt = tt.next0_
        while tt is not None and ((tt.end_char <= max_char or max_char == 0)): 
            t0 = tt
            cell_info = (None)
            ...
            if (cell_info.typ == TableHelper.TableTypes.ROWEND): 
                if (tt != t0): 
                    res.cells.append(TableCellToken._new734(t0, tt, cell_info.row_span, cell_info.col_span))
                res.end_token = tt
                res._eor = True
                break
            if (cell_info.typ != TableHelper.TableTypes.CELLEND): 
