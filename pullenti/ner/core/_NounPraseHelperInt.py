...
class _NounPraseHelperInt:
    ...
    def __try_parse_ru(first : 'Token', typ : 'NounPhraseParseAttr', max_char_pos : int, def_noun : 'NounPhraseItem'=None) -> 'NounPhraseToken':
        ...
        if (noun is None): 
            return None
        res = NounPhraseToken._new621(first, noun.end_token, prep)
        if (adverbs is not None): 
            for a in adverbs: 
        ...
        if (stat is not None): 
            for adj in items: 
                if (adj.morph.items_count > 1): 
                    ...
                    i1 = 0
                    i2 = 0
                    wrapi1623 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat, l1, wrapi1623)
                    i1 = wrapi1623.value
                    wrapi2622 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat, l2, wrapi2622)
                    i2 = wrapi2622.value
                    if (i1 < i2): 
