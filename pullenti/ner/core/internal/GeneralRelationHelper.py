...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new548(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new548(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi551 = RefOutArgWrapper(None)
                inoutres552 = Utils.tryGetValue(all0_, a.name, wrapsi551)
                si = wrapsi551.value
                if (not inoutres552): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli549 = RefOutArgWrapper(None)
                    inoutres550 = Utils.tryGetValue(si, s, wrapli549)
                    li = wrapli549.value
                    if (not inoutres550): 
                        li = list()
                        si[s] = li
