...
class TerminCollection:
    ...
    def __remove_from_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            inoutres756 = Utils.tryGetValue(nod.children, ch, wrapnn755)
            nn = wrapnn755.value
            if (not inoutres756): 
                return
            nod = nn
    ...
    def __find_in_tree(self, key : str, lang : 'MorphLang') -> typing.List['Termin']:
        ...
        while i < len(key): 
            ...
            if (nod.children is not None): 
                wrapnn757 = RefOutArgWrapper(None)
                Utils.tryGetValue(nod.children, ch, wrapnn757)
                nn = wrapnn757.value
            if (nn is None): 
                if (ch == (32)): 
    ...
    def __add_to_hash1(self, key : int, t : 'Termin') -> None:
        li = None
        wrapli758 = RefOutArgWrapper(None)
        inoutres759 = Utils.tryGetValue(self.__m_hash1, key, wrapli758)
        li = wrapli758.value
        if (not inoutres759): 
            li = list()
    ...
    def __try_attach_all_(self, token : 'Token', pars : 'TerminParseAttr'=TerminParseAttr.NO, main_root : bool=False) -> typing.List['TerminToken']:
        ...
        if (tt is not None): 
                ...
                if ((no_vars and tt.term0 is not None and tt.term != tt.term0) and tt.invariant_prefix_length_of_morph_vars <= len(tt.term0)): 
                    ...
                    while i < tt.invariant_prefix_length_of_morph_vars: 
                        ...
                        if (nod.children is None): 
                            ...
                            break
                        nn = None
                        wrapnn762 = RefOutArgWrapper(None)
                        inoutres763 = Utils.tryGetValue(nod.children, ch, wrapnn762)
                        nn = wrapnn762.value
                        if (not inoutres763): 
                            no_vars = True
                            break
            ...
            if (not no_vars): 
                ...
                inoutres769 = self.__manage_var(token, pars, s, nod, len0, wrapres768)
                ...
                while True:
                    ...
                    if (ok): 
                        ...
                        inoutres765 = self.__manage_var(token, pars, wf.normal_case, nod, tt.invariant_prefix_length_of_morph_vars, wrapres764)
                    ...
                    inoutres767 = self.__manage_var(token, pars, wf.normal_full, nod, tt.invariant_prefix_length_of_morph_vars, wrapres766)
                    res = wrapres766.value
                    if (inoutres767): 
                        was_vars = True
        elif (isinstance(token, NumberToken)): 
            wrapres770 = RefOutArgWrapper(res)
            inoutres771 = self.__manage_var(token, pars, str(token.value), root, 0, wrapres770)
            res = wrapres770.value
            if (inoutres771): 
                was_vars = True
        else: 
            return None
        if (not was_vars and s is not None and len(s) == 1): 
            vars0_ = [ ]
            wrapvars772 = RefOutArgWrapper(None)
            inoutres773 = Utils.tryGetValue(self.__m_hash1, ord(s[0]), wrapvars772)
            vars0_ = wrapvars772.value
            if (inoutres773): 
                for t in vars0_: 
                    if (not t.lang.is_undefined): 
                        if (not token.morph.language.is_undefined): 
    ...
    def __manage_var(self, token : 'Token', pars : 'TerminParseAttr', v : str, nod : 'CharNode', i0 : int, res : typing.List['TerminToken']) -> bool:
        ...
        while i < len(v): 
            ...
            if (nod.children is None): 
                return False
            nn = None
            wrapnn774 = RefOutArgWrapper(None)
            inoutres775 = Utils.tryGetValue(nod.children, ch, wrapnn774)
            nn = wrapnn774.value
            if (not inoutres775): 
                return False
            nod = nn
    ...
    def find_termins_by_canonic_text(self, text : str) -> typing.List['Termin']:
        if (self.__m_hash_canonic is None): 
            ...
            for t in self.termins: 
                ct = t.canonic_text
                li = [ ]
                wrapli776 = RefOutArgWrapper(None)
                inoutres777 = Utils.tryGetValue(self.__m_hash_canonic, ct, wrapli776)
                li = wrapli776.value
                if (not inoutres777): 
                    li = list()
                ...
                if (not t in li): 
                    li.append(t)
        res = [ ]
        wrapres778 = RefOutArgWrapper(None)
        inoutres779 = Utils.tryGetValue(self.__m_hash_canonic, text, wrapres778)
        res = wrapres778.value
        if (not inoutres779): 
            return None
        else: 
            return res