...
class DateRelHelper:
    ...
    def __create_date_ex(dr : 'DateReferent') -> typing.List['DateExItemToken']:
        ...
        while dr is not None: 
            ...
            for s in dr.slots: 
                tt = Token(None, 0, 1)
                it = DateExToken.DateExItemToken._new884(tt, tt, DateExToken.DateExItemTokenType.UNDEFINED)
    ...
    def calculate_date_range(dr : 'DateReferent', now : datetime.datetime, from0_ : datetime.datetime, to : datetime.datetime, tense : int) -> bool:
        ...
        det = DateExToken(t, t)
        det.items_from = DateRelHelper.__create_date_ex(dr)
        inoutres909 = det.get_dates(now, from0_, to, tense)
        return inoutres909
    ...
    def calculate_date_range2(dr : 'DateRangeReferent', now : datetime.datetime, from0_ : datetime.datetime, to : datetime.datetime, tense : int) -> bool:
        ...
        if (dr.date_from is None): 
            if (dr.date_to is None): 
                return False
            wrapdt0910 = RefOutArgWrapper(None)
            wrapdt1911 = RefOutArgWrapper(None)
            inoutres912 = DateRelHelper.calculate_date_range(dr.date_to, now, wrapdt0910, wrapdt1911, tense)
            dt0 = wrapdt0910.value
            dt1 = wrapdt1911.value
            if (not inoutres912): 
                return False
            to.value = dt1
            return True
        elif (dr.date_to is None): 
            wrapdt0913 = RefOutArgWrapper(None)
            wrapdt1914 = RefOutArgWrapper(None)
            inoutres915 = DateRelHelper.calculate_date_range(dr.date_from, now, wrapdt0913, wrapdt1914, tense)
            dt0 = wrapdt0913.value
            dt1 = wrapdt1914.value
            if (not inoutres915): 
                return False
            from0_.value = dt0
            return True
        ...
        dt2 = None
        dt3 = None
        wrapdt2916 = RefOutArgWrapper(None)
        wrapdt3917 = RefOutArgWrapper(None)
        inoutres918 = DateRelHelper.calculate_date_range(dr.date_to, now, wrapdt2916, wrapdt3917, tense)
        dt2 = wrapdt2916.value
        dt3 = wrapdt3917.value
        if (not inoutres918): 
            return False
        to.value = dt3
        return True
    ...
    def append_to_string(dr : 'DateReferent', res : io.StringIO) -> None:
        ...
        wrapdt0922 = RefOutArgWrapper(None)
        wrapdt1923 = RefOutArgWrapper(None)
        inoutres924 = DateRelHelper.calculate_date_range(dr, cur, wrapdt0922, wrapdt1923, 0)
        dt0 = wrapdt0922.value
        dt1 = wrapdt1923.value
        if (not inoutres924): 
            return
    ...
    def append_to_string2(dr : 'DateRangeReferent', res : io.StringIO) -> None:
        ...
        inoutres927 = DateRelHelper.calculate_date_range2(dr, cur, wrapdt0925, wrapdt1926, 0)
