...
class ExtOntology:
    ...
    def add(self, ext_id : object, type_name : str, definition_ : str) -> 'ExtOntologyItem':
        ...
        if (rs is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3594(ext_id, rs[0], type_name)
        if (len(rs) > 1): 
            del rs[0]
            res._refs = rs
    ...
    def add_referent(self, ext_id : object, referent : 'Referent') -> 'ExtOntologyItem':
        ...
        if (referent is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3594(ext_id, referent, referent.type_name)
        self.items.append(res)
        return res
    
    def __create_referent(self, type_name : str, definition_ : str) -> typing.List['Referent']:
        analyzer = None
        wrapanalyzer3596 = RefOutArgWrapper(None)
        inoutres3597 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapanalyzer3596)
        analyzer = wrapanalyzer3596.value
        if (not inoutres3597): 
            return None
    ...
    def refresh(self, item : 'ExtOntologyItem', definition_ : object) -> bool:
        ...
        analyzer = None
        wrapanalyzer3598 = RefOutArgWrapper(None)
        inoutres3599 = Utils.tryGetValue(self.__m_anal_by_type, item.type_name, wrapanalyzer3598)
        analyzer = wrapanalyzer3598.value
        if (not inoutres3599): 
            return False
    ...
    def _get_analyzer_data(self, type_name : str) -> 'AnalyzerData':
        a = None
        wrapa3600 = RefOutArgWrapper(None)
        inoutres3601 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapa3600)
        a = wrapa3600.value
        if (not inoutres3601): 
            return None
    ...
    def __init_hash(self) -> None:
        with self.__m_lock: 
            ...
            for it in self.items: 
                if (it.referent is not None): 
                    ont = None
                    wrapont3603 = RefOutArgWrapper(None)
                    inoutres3604 = Utils.tryGetValue(hash0_, it.referent.type_name, wrapont3603)
                    ont = wrapont3603.value
                    if (not inoutres3604): 
                        ont = IntOntologyCollection._new3602(True)
    ...
    def attach_referent(self, r : 'Referent') -> typing.List['ExtOntologyItem']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3605 = RefOutArgWrapper(None)
        inoutres3606 = Utils.tryGetValue(self.__m_hash, r.type_name, wraponto3605)
        onto = wraponto3605.value
        if (not inoutres3606): 
            return None
        li = [ ]
    ...
    def attach_token(self, type_name : str, t : 'Token') -> typing.List['IntOntologyToken']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3607 = RefOutArgWrapper(None)
        inoutres3608 = Utils.tryGetValue(self.__m_hash, type_name, wraponto3607)
        onto = wraponto3607.value
        if (not inoutres3608): 
            return None
