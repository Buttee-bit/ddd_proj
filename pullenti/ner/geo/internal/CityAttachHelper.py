...
class CityAttachHelper:
    ...
    def try_define(li : typing.List['CityItemToken'], ad : 'GeoAnalyzerData', always : bool=False) -> 'ReferentToken':
        ...
        if (res is None and len(li) > 1): 
            ...
            if (res is not None and res.end_char <= li[1].end_char): 
                res = (None)
        if (res is None): 
            wrapoi1436 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try1(li, wrapoi1436, ad)
            oi = wrapoi1436.value
        if (res is None): 
            wrapoi1437 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1437, False)
            oi = wrapoi1437.value
        if (res is None): 
            wrapoi1438 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_name_exist(li, wrapoi1438, False)
            oi = wrapoi1438.value
        if (res is None): 
            res = CityAttachHelper.__try4(li)
        if (res is None and always): 
            wrapoi1439 = RefOutArgWrapper(None)
            res = CityAttachHelper.__try_noun_name(li, wrapoi1439, True)
            oi = wrapoi1439.value
        if (res is None and always): 
            if (OrgItemToken.try_parse(li[0].begin_token, ad) is not None): 
                pass
            else: 
                wrapoi1440 = RefOutArgWrapper(None)
                res = CityAttachHelper.__try_name_exist(li, wrapoi1440, True)
                oi = wrapoi1440.value
    ...
    def __try1(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', ad : 'AnalyzerDataWithOntology') -> 'ReferentToken':
        ...
        if (oi.value is None): 
            ...
            rt = ReferentToken._new950(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
        elif (typ is None): 
            typ = oi.value.typ
        if (rt is None): 
    ...
    def __try_noun_name(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
            ...
            if (not li[0].doubtful): 
                ...
                if (npt is not None): 
                    if (npt.end_token.end_char > li[i1].end_char and len(npt.adjectives) > 0 and not npt.adjectives[0].end_token.next0_.is_comma): 
                        ...
                        if (len(li2) > 1): 
                            oi2 = None
                            wrapoi21444 = RefOutArgWrapper(None)
                            inoutres1445 = CityAttachHelper.__try_noun_name(li2, wrapoi21444, False)
                            oi2 = wrapoi21444.value
                            if (inoutres1445 is not None): 
                                ok = True
        ...
        res = ReferentToken._new950(city, li[0].begin_token, li[len(li) - 1].end_token, mc)
        num = None
    ...
    def __try_name_exist(li : typing.List['CityItemToken'], oi : 'IntOntologyItem', always : bool) -> 'ReferentToken':
        ...
        city._add_misc(li[0].misc)
        city._add_misc(li[0].misc2)
        return ReferentToken._new950(city, li[0].begin_token, Utils.ifNotNull(end_tok, li[0].end_token), li[0].morph)
