...
class ProcessorService:
            ...
            if (img is not None): 
                for kp in img.items(): 
                    if (not kp[0] in ProcessorService.__m_images): 
                        ProcessorService.__m_images[kp[0]] = ImageWrapper._new3632(kp[0], kp[1])
    ...
    def get_image_by_id(image_id : str) -> 'ImageWrapper':
        ...
        if (image_id is not None): 
            res = None
            wrapres3633 = RefOutArgWrapper(None)
            inoutres3634 = Utils.tryGetValue(ProcessorService.__m_images, image_id, wrapres3633)
            res = wrapres3633.value
            if (inoutres3634): 
                return res
        if (ProcessorService.__m_unknown_image is None): 
            ProcessorService.__m_unknown_image = ImageWrapper._new3632("unknown", PullentiNerCoreInternalResourceHelper.get_bytes("unknown.png"))
    ...
    def add_image(image_id : str, content : bytearray) -> None:
        ...
        if (image_id is None): 
            return
        wr = ImageWrapper._new3632(image_id, content)
