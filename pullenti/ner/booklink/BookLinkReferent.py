...
class BookLinkReferent(Referent):
    ...
    def year(self) -> int:
        """ Год """
        year_ = 0
        wrapyear526 = RefOutArgWrapper(0)
        inoutres527 = Utils.tryParseInt(Utils.ifNotNull(self.get_string_value(BookLinkReferent.ATTR_YEAR), ""), wrapyear526)
        year_ = wrapyear526.value
        if (inoutres527): 
            return year_
        else: 
            return 0
