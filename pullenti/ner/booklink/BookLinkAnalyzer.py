...
class BookLinkAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (re.number is not None): 
                li = [ ]
                wrapli516 = RefOutArgWrapper(None)
                inoutres517 = Utils.tryGetValue(refs_by_num, re.number, wrapli516)
                li = wrapli516.value
                if (not inoutres517): 
                    li = list()
        ...
        while True:
            ...
            li = [ ]
            wrapli518 = RefOutArgWrapper(None)
            inoutres519 = Utils.tryGetValue(refs_by_num, Utils.ifNotNull(re.number, ""), wrapli518)
            li = wrapli518.value
            if (not inoutres519): 
                continue
            i = 0
    ...
    def __try_parse(t : 'Token', is_in_lit : bool, max_char : int=0) -> typing.List['ReferentToken']:
        ...
        next_num = None
        nn = 0
        wrapnn524 = RefOutArgWrapper(0)
        inoutres525 = Utils.tryParseInt(Utils.ifNotNull(num, ""), wrapnn524)
        nn = wrapnn524.value
        if (inoutres525): 
            next_num = str((nn + 1))
        ...
        if (tn1 is None and start_of_name is None): 
            if (is_electr_res): 
                ...
                rts0 = list()
                bref0 = BookLinkRefReferent._new520(uri_re)
                if (num is not None): 
                    bref0.number = num
            ...
            if (decree is not None and num is not None): 
                rts0 = list()
                bref0 = BookLinkRefReferent._new520(decree.get_referent())
                if (num is not None): 
                    bref0.number = num
            ...
            if (book_prev is not None): 
                ...
                if (blt0 is not None and blt0.typ == BookLinkTyp.PAGERANGE): 
                    rts0 = list()
                    bref0 = BookLinkRefReferent._new520(book_prev)
                    if (num is not None): 
                        bref0.number = num
        ...
        rts = list()
        bref = BookLinkRefReferent._new520(res)
        if (num is not None): 
            bref.number = num
