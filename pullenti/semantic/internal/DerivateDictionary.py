...
class DerivateDictionary:
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos3701 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos3701)
                pos = wrappos3701.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos3705 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos3705)
            pos = wrappos3705.value
            while cou > 0: 
                wrappos3703 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos3703)
                pos = wrappos3703.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos3702 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos3702)
                    pos = wrappos3702.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos3704 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos3704)
            pos = wrappos3704.value
        self.__m_buf = wr
