...
class Sentence(object):
    ...
    def __create_lists(self, s : 'NGSegmentVariant') -> None:
        ...
        first_pass4355 = True
        while True:
            if first_pass4355: first_pass4355 = False
            else: i += 1
    ...
    def __create_links(self, s : 'NGSegmentVariant') -> None:
        ...
        first_pass4356 = True
        while True:
            if first_pass4356: first_pass4356 = False
            else: i += 1
            if (not (i < len(s.links))): break
            link0 = s.links[i]
            if (link0 is None): 
                ...
                if (li.to is not None and li.to.res_object is not None): 
                    ...
                    if (li.typ == NGLinkType.BE): 
                        if ((isinstance(li.from0_.source.source, NumbersWithUnitToken)) and li.to is not None): 
                            ...
                            continue
                        be = SemObject._new3713(gr, SemObjectType.VERB)
    ...
    def __create_result(self, blk : 'SemBlock') -> None:
                ...
                if (isinstance(it.source, NounPhraseToken)): 
                    ...
                    if (npt.multi_nouns and it.result.quantity is None): 
                        ...
                        while i < len(npt.adjectives): 
                            so = SemObject._new3713(it.res_graph, it.result.typ)
        ...
        first_pass4357 = True
        while True:
            if first_pass4357: first_pass4357 = False
            else: i += 1
        ...
        for it in self.items: 
            if (it.result is not None and it.typ == SentItemType.VERB and (isinstance(it.source, VerbPhraseToken))): 
                if (agent is not None): 
                    ...
                    for li in it.res_graph.links: 
                        if (li.typ == SemLinkType.PACIENT and li.source == it.result): 
                            ...
                            break
                    if (not has_pac): 
                        ni0 = NGItem._new3706(agent)
                        gli0 = NGLink._new3719(ni0, Utils.asObjectOrNull(it.source, VerbPhraseToken), NGLinkType.PACIENT)
                ...
                if (agent is None): 
                    continue
                ni = NGItem._new3706(agent)
                gli = NGLink._new3719(ni, Utils.asObjectOrNull(it.source, VerbPhraseToken), NGLinkType.AGENT)
                if (agent.result_list is not None): 
        ...
        first_pass4358 = True
        while True:
            if first_pass4358: first_pass4358 = False
            else: i += 1
            if (not (i < len(self.items))): break
            it = self.items[i]
            ...
            if (link is None): 
                ...
                first_pass4359 = True
                while True:
                    if first_pass4359: first_pass4359 = False
                    else: j += 1
    ...
    def parse_variants(t0 : 'Token', t1 : 'Token', lev : int, max_count : int=0, regime : 'SentItemType'=SentItemType.UNDEFINED) -> typing.List['Sentence']:
        ...
        first_pass4360 = True
        while True:
            if first_pass4360: first_pass4360 = False
            else: t = t.next0_
            ...
            for it in items_: 
                nexts = None
                wrapnexts3722 = RefOutArgWrapper(None)
                inoutres3723 = Utils.tryGetValue(m_nexts, it.end_token.end_char, wrapnexts3722)
                nexts = wrapnexts3722.value
                if (not inoutres3723): 
    ...
    def calc_coef(self, no_result : bool) -> None:
        ...
        first_pass4361 = True
        while True:
            if first_pass4361: first_pass4361 = False
            else: i += 1
            if (not (i < len(self.items))): break
            it = self.items[i]
    ...
    @staticmethod
    def _new3736(_arg1 : 'NGLinkType') -> 'Sentence':
        res = Sentence()
